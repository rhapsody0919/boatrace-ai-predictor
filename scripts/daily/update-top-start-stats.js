...
function aggregateTopStarts(startTimings, raceResults) {
  ...
  // { "venue-boatNumber": { raceCount, topStartCount, winWhenTopStartCount } }
  const stats = new Map();
  ...
  byRace.forEach((rows, raceId) => {
    ...
    const topStartCandidates = validRows.filter(
      (r) => r.start_timing === minStartTiming,
    );
    if (topStartCandidates.length !== 1) return; // 同着はスキップ

    const topStart = topStartCandidates[0];
    bump(venueCode, topStart.boat_number, "topStartCount");

    const winnerBoatNumber = rank1ByRace.get(raceId);
    if (winnerBoatNumber === topStart.boat_number) {
      bump(venueCode, topStart.boat_number, "winWhenTopStartCount");
    }
  });
  ...
  return [...stats.values()].map((s) => ({
    ...
    top_start_count: s.topStartCount,
    top_start_rate: parseFloat(
      ((s.topStartCount / s.raceCount) * 100).toFixed(2),
    ),
    win_count_when_top_start: s.winWhenTopStartCount,
    win_rate_when_top_start:
      s.topStartCount > 0
        ? parseFloat(
            ((s.winWhenTopStartCount / s.topStartCount) * 100).toFixed(2),
          )
        : 0,
...
async function upsertTopStartStats(records) {
...
async function main() {
  console.log("=== Top Start Stats 日次更新 ===");
  ...
  const records = aggregateTopStarts(startTimings, raceResults);
  console.log(`\n集計完了: ${records.length}件（会場×枠番）`);

  const totalInserted = await upsertTopStartStats(records);
