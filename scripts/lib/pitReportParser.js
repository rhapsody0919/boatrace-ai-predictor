 ...
 *   - 対象レースでコメントが未公開のとき（2026-09-21 11:30 JST、発走5時間前の実ページで確認）: 艇ごとの表とレポーター名は
 *     あるが、コメント欄が空（&nbsp;）・前走のリンクの中身も空。「対象外」のメッセージは出ない。status = "target_pending"
 *     （表そのものが無く、メッセージも無い場合も、見出しとレース番号があれば target_pending。構造の別の形への備え）
 *
