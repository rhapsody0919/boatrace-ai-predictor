/**
 * オリジナル展示・モーター使用開始日の保存先（マイグレーション087）が、接続先のDBに適用済みかの判定。
 *
 * 目的: 087の適用前にコードをデプロイ・有効化しても、壊れない・意味の違う書き込みをしない。テーブルが無いDBでは、
...
export async function detectBoatcastSchema(
  ...
  for (const table of tables) {
    ...
    if (!isScrapeSchemaMissingError(error, [table])) {
      ...
      definite = false;
      warn(
        `⚠️ マイグレーション087の適用状況を確認できません（書き込みません）: ${error.message}`,
      );
    }
  }
