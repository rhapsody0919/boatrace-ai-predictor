 ...
 * API自体はステータスによる制限を設けない（汎用機能）が、status='posted'の
 * 下書きについてはフロント側（DraftCard）でボタンを表示しない。postedを
 * archivedにすると動画保持ポリシー（ADR 0022）の保持期間が30日→7日に
 * 短縮されてしまうため（2026-08-31発見・対応）。
 */
