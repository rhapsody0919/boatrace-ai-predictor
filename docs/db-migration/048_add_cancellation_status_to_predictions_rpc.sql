-- get_predictions_by_date / get_predictions_by_date_light に cancellationStatus を追加する（BOA-254）。
--
-- 背景: /venue/:code 等の会場別レース一覧・レース詳細ページは、Edge API
-- （api/predictions/[date].js）経由でこれらのRPC関数を最優先で呼び出しており、
-- src/services/supabaseDataService.js の直接クエリ（フォールバック経路）にのみ
-- cancellation_status を追加しても、通常運用（Edge API成功時）ではこのRPCの
-- 出力がそのまま使われるため中止バッジ・バナーが一切反映されない
-- （047マイグレーションでracesテーブルに追加したカラムをRPCの出力に含める）。
--
-- 041時点の完全な定義（pg_get_functiondefで確認済み）をベースに、
