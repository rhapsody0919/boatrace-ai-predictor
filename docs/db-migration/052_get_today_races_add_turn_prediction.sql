-- get_today_races RPC関数に展開予測（turnPrediction）を追加
--
-- 背景: BOA-172（トップページのイン崩れ注意度ハイライト）のレビューで、
-- 「発走時刻・展開予測も表示した方が情報として充実する」との指摘を受けた。
-- 発走時刻（startTime）は既にRPCが返しているためフロント側の表示追加のみで対応可能だが、
-- 展開予測（predictions.feature_contributions.turnPrediction）はこれまで
-- get_today_races()が返していなかったため、034に続けて追加する。
--
-- turnPredictionのうち先頭パターン（patterns[0]、最も確率が高い1着想定）のみを
-- 抜き出して返す。フロント側では技（技術）・想定1着コース・確率のみを使い、
-- 詳細な確率分布（secondPlace/thirdPlace等）は今回のホーム画面用途では不要なため含めない
-- （必要になった場合は別途追加を検討）。
