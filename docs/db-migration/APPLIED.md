...
| 093 | 093_race_special_notes_unique_by_racer.sql | 適用済み | 2026-09-23 06:3x JSTに、親がユーザーの承認のもと、Management API経由で、1トランザクション（`BEGIN`〜`COMMIT`）で適用。適用前に本番0件を確認。race_special_notes の一意索引 (venue_code, race_date, category, detail_text) に racer_name を追加し、同日・同内容の別選手の通知が1行に潰れる問題を解消（BOA-371）。適用後の確認: racer_name列(text)が存在、一意制約が5列(venue_code, race_date, category, detail_text, racer_name)であることを確認済み。対応コード: scripts/daily/scrape-race-information.js（onConflictを5列に変更） |
| 097 | 097_limit_trg_update_predictions_columns.sql | 適用済み | 2026-09-24 15:0x JSTに、**ユーザー自身がSupabase Dashboard SQL Editorで**適用（Supabase MCPが`--read-only`固定・Management API経由のDDLもauto modeでブロックされたため、今回は親がManagement API経由で適用する従来パターンではなくユーザーが直接実行）。`trg_update_predictions`の発火条件を`AFTER INSERT OR UPDATE ON race_results`（列指定なし）から`UPDATE OF rank1, rank2, rank3, payout_win, payout_place_1, payout_place_2, payout_trifecta, payout_trio`に限定（BOA-409、WS8(c)-2）。適用後の確認: `pg_get_triggerdef`で上記8列限定の定義になっていることを確認済み |
