...
| 081 | 081_race_entries_racelist_fields.sql | 未適用 | 出走表の全項目化（docs/design/pre-race-full-fields/plan.md）。race_entries に weight_kg・branch・hometown・f_count・l_count・is_absent、race_conditions に race_distance_m・race_labels を追加する案。NULL可・DEFAULTなし（メタデータのみの変更）。本番へは未適用（ユーザーの承認後に適用）。コードは、列が無い間は旧実装と同じ列だけを書く（scripts/lib/preRaceSchema.js） |
| 082 | 082_exhibition_data_beforeinfo_fields.sql | 未適用 | 直前情報の全項目化（docs/design/pre-race-full-fields/plan.md）。exhibition_data に exhibition_course・start_flag・prev_finish_mark・is_absent を追加する案。NULL可・DEFAULTなし。本番へは未適用（ユーザーの承認後に適用）。コードは、列が無い間は旧実装と同じ列だけを書く |
| 085 | 085_race_pit_reports.sql | 未適用 | ピットレポート（選手コメント）の保存先 race_pit_reports（レース単位）・race_pit_comments（艇単位）の新設案（BOA-379、docs/design/pit-comments/）。新規テーブルのみ・RLS有効・匿名の権限なし。本番へ適用する前に、ユーザーの承認が要る。適用順は 085 → コードの shadow → live（未適用のDBでは、liveのスロットが error になる） |
