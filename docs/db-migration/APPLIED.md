...
| 103 | 103_restore_today_races_race_conditions_fields.sql | 適用済み | `get_today_races()` から欠落した `seriesDay`・`isFinalDay`・`raceTitle`・`raceStage` と `LEFT JOIN race_conditions` を復旧する（[BOA-431](https://linear.app/boat-ai/issue/BOA-431)）。102（当初063、PR #824）が068ではなくそれ以前の定義を土台に `CREATE OR REPLACE` したため、068が入れていた4キーが丸ごと消えていた。2026-09-25の本番実測（`pg_get_functiondef`）で `has_rc_join=false` / `has_series_day=false` を確認。**RaceCard.jsx の優勝戦・準優勝戦バッジが本番で表示されなくなっていた**。内容は068の `get_today_races` の定義そのまま（068以降にこの関数を変えたのは102だけであることを確認済み）。BOA-363と同型の回帰。適用後は `node --env-file=.env.local scripts/maintenance/verify-rpc-output-keys.js` で確認する | **2026-09-25 にユーザー自身がSupabase Dashboard SQL Editorで適用**（Supabase MCPが`--read-only`固定・service keyでの書き込みもauto modeがブロックするため、97・100と同じ手続き）。適用後の実測: `verify-rpc-output-keys.js` が `get_today_races`（144レース）を含む全RPCで `[OK]`。本番API `https://www.boat-ai.jp/api/races/today` でも144レース全件に `raceTitle`・`seriesDay`・`isFinalDay`・`raceStage` が返ることを確認（`isFinalDay=true` は24件＝最終日の会場、`raceStage` は144件に値あり）。RaceCard.jsx の優勝戦・準優勝戦バッジが復旧した（関数定義の長さは 3487→3721 に増加。本番画面 https://www.boat-ai.jp/races/2026-09-25/01 の桐生11R・12Rで「🏆 優勝戦」バッジの表示を実視認）。 以後この型の回帰は、PR時に `verify:rpc-key-regression`（SQL同士の静的比較、ADR-0074）が、本番の実物は `nightly-verify-db.yml`（毎晩JST3:00、ADR-0075のPR #846）が検知する。
