...
| 076 | 076_enable_rls_on_public_tables.sql | 未適用 | BOA-370。対象16テーブルのRLS有効化・公開SELECTポリシー8本、anon/authenticatedの書き込み権限の剥奪（全テーブル・ビュー）、ビュー4本のsecurity_invoker化、update_venue_stats()のEXECUTE剥奪、既定権限の抑止。本番への適用はユーザー承認後（適用後の確認SQLは同ファイル末尾。適用したら「適用済み」に更新し、確認SQLの結果を書く） |
