...
| 071 | 071_add_scraped_timestamps.sql | 未適用 | 適用先: exhibition_data・race_entries・race_start_timings の created_at・updated_at（timestamptz、NULL可）。created_at にだけ DEFAULT now()（既存行はNULLのまま）。適用が先、マージが後（WS2） |
