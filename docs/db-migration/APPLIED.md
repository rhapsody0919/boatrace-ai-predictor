...
| 075 | 075_scrape_slots_and_job_state.sql | 未適用 | 2026-09-20時点で未適用（WS4a・T4a-02のマイグレーション案。本番への適用はユーザーの承認後、T4a-03）。適用後の確認: テーブル scrape_slots・scrape_job_state、列 race_odds.window_min・source、索引 uq_race_odds_race_window・idx_scrape_slots_active、関数 ensure_scrape_slots・claim_scrape_slots（anonのEXECUTE無し）、両テーブルのRLS有効。確認SQLはマイグレーション末尾のコメント |
