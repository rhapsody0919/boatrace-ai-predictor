...
| 086 | 086_race_pit_reports_public_read.sql | 未適用 | 上記2表を匿名（画面）から読めるようにする SELECT のみのポリシーと GRANT（公式コンテンツの再表示を始める境目。docs/adr/0067 の追記案。画面の実装・モック承認の後に適用）。085 が先 |
| 092 | 092_claim_scrape_slots_by_offset.sql | 未適用 | 新関数 claim_scrape_slots_by_offset を追加（既存の claim_scrape_slots・075のテーブル・列は変更しない）。オッズの発走60分前の窓（-60）だけ、未公開の間、許容幅を3分から30分（-30の窓が始まるまで）に延ばす（完了の定義Bの見直し。BOA-386続き、tasks.md T4b-24）。p_grace_by_offset（例 `{"-60":30}`）が空のときは claim_scrape_slots と同じ結果（`npm run verify:scrape-slots-sql` の差分テストで確認）。権限は claim_scrape_slots と同じ（service_roleのみEXECUTE）。コード（store.js）は、この関数が無い（PGRST202）ときだけ既存の claim_scrape_slots へ自動フォールバックする（延長は効かないが取得は止まらない）ため、適用前にコードをマージ・デプロイしても無害。適用後: pg_proc に関数が1件増え、anon・authenticated はEXECUTEできず、service_roleのみ実行できることを確認して、この行を「適用済み」に更新する |
| 091 | 091_boatcast_original_exhibition.sql | 未適用 | BOATCASTのオリジナル展示（N25）・モーター使用開始日（N26）の保存先: race_original_exhibition・race_original_exhibition_values・venue_motor_start_dates の3表を新設（RLS有効・anonの権限なし。公式コンテンツの再表示を含むため匿名のSELECTは付けない。ADR-0067）。コード（api/cron/boatcast-oriten.js・boatcast-motor-start.js）は、mode が off（行なし）の間は動かず、091が未適用のDBでは live でも書かずに error にする。適用後: 3表が存在しRLS有効・anonのSELECT権限なし。docs/design/boatcast-original-exhibition/ |
| 090 | 090_motor_pretest_stats.sql | 未適用 | 新規テーブル motor_pretest_stats（前検タイム・前検順位・節時点のモーター/ボートの番号と2連対率の日次スナップショット。公式 race/rankingmotor。N23、tasks.md T4b-20。設計: docs/design/scraping-vercel-consolidation/plan.md §15）。RLS有効・anon/authenticatedの全権限を剥奪（書き込みはservice_roleのみ。読み手ができたらSELECTポリシーを追加）。適用前は、ジョブ motor_pretest は off のまま（`scrape_job_state`に行を作らない）で、live にすると成功にせず失敗する（090が未適用のため）。適用後: 本番実測で、テーブル・RLS有効・anonのSELECT権限なしを確認して、この行を「適用済み」に更新する |
| 089 | 089_data_health_functions.sql | 未適用 | 汎用の日次監視（data_health、完了の定義C）が呼ぶ、読み取り専用の集計関数7本（`data_health_coverage`・`data_health_pre_race_fields`・`data_health_pit_reports`・`data_health_race_series`・`data_health_racer_period_stats`・`data_health_monthly_result`・`data_health_table_rows`）。テーブル・データの変更なし。SECURITY INVOKER・STABLE・service_roleのみEXECUTE。適用前でも、ジョブは既定`off`で挙動不変。適用後の確認SQLはファイル冒頭。087・088は他のデータセットに割当済みのため、番号を空けている |
