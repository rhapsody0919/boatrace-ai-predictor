...
| 072 | 072_kb_archive_tables.sql | **未適用（案。ユーザー承認待ち）** | 新規テーブル kb_archive_venue_days・kb_archive_races・kb_archive_boats（K/Bファイル長期バックフィル用。設計: docs/design/kb-longterm-backfill/plan.md）。適用後は information_schema.tables に3表が存在し、pg_class.relrowsecurity が3表とも true になること |
