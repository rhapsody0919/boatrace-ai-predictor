...
| 083 | 083_racer_period_stats.sql | 未適用 | 案（本番へ未適用）。テーブル racer_period_stats（選手×期の成績、fan由来、RLS有効・anon権限なし）と、racer_profiles の列 sex・training_term を追加する。設計: docs/design/racer-period-stats/plan.md。適用前に、長時間クエリが0件であることを確認する |
| 084 | 084_race_series.sql | 未適用 | 案（本番へ未適用）。テーブル race_series（節: 節名・グレード・種別・開始日・終了日・総日数、月間スケジュール由来、RLS有効・anon権限なし）を新設する。設計: docs/design/racer-period-stats/plan.md |
