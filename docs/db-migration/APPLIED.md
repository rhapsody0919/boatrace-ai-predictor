...
| 074 | 074_kb_archive_tables.sql | **未適用（案。ユーザー承認待ち）** | 新規テーブル kb_archive_venue_days・kb_archive_races・kb_archive_boats（K/Bファイル長期バックフィル用。設計: docs/design/kb-longterm-backfill/plan.md）。適用後は information_schema.tables に3表が存在し、pg_class.relrowsecurity が3表とも true になること |
...
| 077 | 077_race_start_timings_boat_result.sql | **未適用（案。ユーザー承認待ち）** | race_start_timings に列 finish_mark・finish_rank・entry_course・race_seconds を追加（艇別の結果。設計: docs/design/race-result-full-fields/plan.md）。適用後は information_schema.columns に4列が存在すること。コードは、列が現れてから約5分後に全艇の行を書き始める（それまでは旧形式） |
| 078 | 078_race_results_status_refund.sql | **未適用（案。ユーザー承認待ち）** | race_results に列 race_status・refund_boats・remark を追加（返還・不成立の識別。設計: 同上）。適用後は information_schema.columns に3列が存在すること。行のUPDATEは発生しない（trg_update_predictions は発火しない） |
| 079 | 079_race_payouts.sql | **未適用（案。ユーザー承認待ち）** | 新規テーブル race_payouts（払戻明細。設計: 同上）。適用後は information_schema.tables に存在し、pg_class.relrowsecurity が true、anon に SELECT 権限が無いこと |
| 080 | 080_racer_news_pending.sql | **未適用（案。ユーザー承認待ち）** | 新規テーブル racer_news_pending（選手ニュース自動収集の要確認リスト。pending.json の後継。T4b-15。設計: docs/design/scraping-vercel-consolidation/tasks.md）。適用後は information_schema.tables に存在し、pg_class.relrowsecurity が true、anon の SELECT 権限が無い（has_table_privilege('anon','racer_news_pending','SELECT') が false）こと。`racer_news` ジョブを live にする前に適用する |
