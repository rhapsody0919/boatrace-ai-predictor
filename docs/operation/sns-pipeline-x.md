...
**「◯日目」は対象レースの暦日（`race_id`先頭のYYYY-MM-DD部分）で数える。エントリの通し番号（1件目/2件目...）をそのまま日数に転用しない**（2026-09-09、実際に発生した不具合を受けて追記）。イン崩れ注意度99%以上のレースは1日に複数出ることがあり、実際に2026-09-09にはびわこ2R・桐生2Rの2レースが同日に対象になった。この2件目（桐生2R）を「2件目」＝「2日目」と誤って扱い、実際は企画1日目なのに「2日目も不的中でした」という誤った本文・画像を生成してしまった実例がある。正しくは、同じ企画に紐づく全エントリの`race_id`日付部分を集め、重複を除いた日付の昇順で「この対象レースの日付が何番目の日か」を数える。同日に複数レースがある場合は「◯日目・◯レース目」のように両方を明記する（例: 2026-09-09の2レース目なら「1日目・2レース目」、`sns_campaigns.start_date`は企画作成日であり対象レースの初日と一致するとは限らないため、日数計算の起点には使わない）。
...
1. `scripts/lib/contentChannels/renderCampaignCard.js`の`renderCampaignEntryCard()`で1枚目（買い目・収支カード）を生成する。
   ...
   - **`variant: 'result'`の`headline`は`variant: 'picks'`の見出しを使い回さない**（v6、2026-09-09追加）。ピック時点の見出し（例:「初日は残念ながら不的中でした」の元になった導入文）をそのまま再利用すると、結果カードなのに事前予想の文言が残り読者が混乱する。結果カードの見出しは`hit`の値を踏まえて必ず書き直す（例: 不的中なら「初日は不的中でした。正直に記録します」、的中なら「2レース目は的中！通算収支がプラスに転じました」。「◯日目」ではなく「◯レース目」等、上記の日付計算ルールと矛盾しない表現を選ぶ）
