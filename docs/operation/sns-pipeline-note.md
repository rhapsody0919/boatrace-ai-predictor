...
`sns_topics.campaign_id`が設定されている場合でも、本Routine自身が`getCampaignEntries()`で過去エントリを取得する必要は無い。継続性のある本文（前回までの結果を踏まえた導入等）は2.で取得するブログ本文側で既に反映済み（`sns-pipeline-x.md`「2. 企画（キャンペーン）由来のネタの場合」参照）のため、3.の変換ではそれをそのままnote向けフォーマットに変換すればよい。
