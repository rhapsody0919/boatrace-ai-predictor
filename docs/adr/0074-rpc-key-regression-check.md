...
- **`verify-rpc-output-keys.js` を夜間ワークフローでsecrets付き実行する**: 本番DBへ接続するワークフローを増やす影響が大きい。かつ「本番に入った後」でしか検知できないため、予防にならない。ファイル比較でPR時に止めるほうが早い。ADR-0072 で先送りにした夜間実行の必要性は、この検査の導入で下がった。
