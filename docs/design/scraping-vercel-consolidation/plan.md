...
| `/api/cron/races-init` | `*/2 20-23,0-14 * * *` | 05:00〜23:58、2分 | チャンク | A8。会場チャンクを、終わるまで進める（§4.2）。終わった日は、予測ロジックの変更検知（ハッシュの比較）のみ |
...
| `exhibition`（A2） | `-33`（展示公開の最初の機会）、期限は`-7`まで。**加えて、`10`（発走の10分後〜36分後）の窓の外の補完のスロット**（BOA-382。取得済みは、DBの読み取りだけ。runbook Q-8） | 26（`-33`から`-7`） | 120秒 | 90秒 | 24件・4 | `exhibition_time`が非NULLで存在する（既にあれば取得せず`skipped_have_data`）。未公開は`no_values`で再試行 |
...
## 15. 前検タイム（N23）と日次照合（N29）

対応: tasks.md T4b-20・T4b-21 / 手順: [verification-runbook.md](./verification-runbook.md) T / データ項目: [data-catalog.md](./data-catalog.md) N23・N29 / 全体設計: [optimal-scraping-design.md](./optimal-scraping-design.md) §2.3・§2.6・§4.2（順序5）。第1弾の追加項目。既定は`off`（マージしても本番の挙動は変わらない）。

### 15.1 前検タイム・前検順位・節時点のモーター/ボート2連対率（N23）

**実ページの構造（2026-09-21、13ページで確認。フィクスチャ7件は`scripts/lib/__fixtures__/motorPretest/`）**

| 項目 | 実測 |
|---|---|
| URL | `race/rankingmotor?jcd=会場2桁&hd=YYYYMMDD`。会場×日で1ページ。過去日も取得できる（2025-12-03・04・05で確認） |
| 表 | 節の全選手（約45人。開催日に走らない選手を含む。代替選手がいると52人の例もある）。列: 順位・登録番号・ボートレーサー・級別・モーター（番号・2連対率）・ボート（番号・2連対率）・前検タイム |
| 「順位」 | **表示中の並び順の順位**。既定の並びはモーター2連対率の降順で、前検順位ではない。前検タイムの昇順のページ（sort=5）の順位と、前検タイムから計算した順位（同じ値は同順位、次は飛ぶ）が、45人全員で一致することを確認した（検証: `verify:motor-pretest`）。パーサーは順位列を読まず、前検タイムから計算する |
| 開催が無い日・会場 | 表が無く「データがありません。」（13KB）。パーサーは`no_data`とし、開催のある会場×日でこれが返れば異常として扱う |
| 見出し | 開催名（全角。NFKCで正規化）・会場（`text_place2_NN.png`）・日付（タブの`hd=`）。パーサーは要求した会場・日付と照合し、違えば保存しない |

**既存列との重複（要確認とされていた点）**: 本番の`race_entries`（4ページ・183選手の突合。同じ会場・日の出走表）と比べた結果は次のとおり。**このページにしかない新しい情報は、前検タイムのみ**。

| 項目 | 一致 |
|---|---|
| モーター番号 | 100%（183/183） |
| ボート番号 | 99%（181/183。不一致2件は、当日のボート交換） |
| モーター2連対率 | 差0.1以内が100%（ページは小数第1位、`race_entries`は第2位） |
| ボート2連対率 | 差0.1以内が99%（181/183。外れ2件は、ボート番号が違う選手＝ボートを交換した選手） |

2連対率・番号は要求（N23の取得項目）どおり保存するが、低精度の重複であり、分析には`race_entries`を使う。

**方式**（日次スナップショット。`kind: daily`）
...
erDiagram
    motor_pretest_stats {
        ...
        numeric boat_2rate
        numeric pretest_time
        smallint pretest_rank
        text series_title
        timestamptz created_at
        timestamptz updated_at
    }
...
外部キーは持たない（節の全選手が載り、`racer_profiles`に無い新人も載りうるため、書き込みを他テーブルに依存させない）。読み手は、選手・会場ごとの直近の行（`race_date`の降順）を引く（as-of結合）。

### 15.2 日次の照合（N29）

**方式の変更（独立レビュー）**: 当初案（公式の`resultlist`・`pay`の新規パーサー）は、「新規パーサーが最重で、`pay`は日付指定が未確認」と指摘された。既存の`kfile_sync`が毎日ダウンロードしているKファイル（別ドメイン`www1.mbrace.or.jp`。公式の競走成績）と、`scripts/lib/raceResultAudit.js`（`compareRaceWithKDay`等）を使う照合に変更した。**新しい取得先・新しいテーブルは作らない**。

**突合**（`scripts/lib/dailyReconcile.js`。前日Dの1レースごと）

| 項目 | DB | K | 種類 |
|---|---|---|---|
| 結果の有無 | `race_results`の行 | レースの結果 | `result_missing`（Kにあり、DBに無い）・`result_not_in_k`（逆）・`race_missing_in_db`（`races`に無い）・`cancelled_but_in_k`（確定中止なのにKに結果がある） |
| 着順 | rank1〜3・rank4〜6 | 完走艇の着順 | `rank123`・`rank456`（既存の`compareRaceWithKDay`）。`rank456_unfilled`は同期待ち |
| 払戻 | 旧15列（勝式別。同着の複数口・特払・不成立を扱う） | 払戻明細 | `payout`・`refund_payout`（不成立の返還額¥100） |
| 払戻明細 | `race_payouts`（明細のあるレースのみ） | 払戻明細 | `payout_table` |
| 進入 | `actual_course_1〜6` | 進入の欄 | `course`。全てNULLは`course_unfilled`（同期待ち） |

**独立性の限界**: rank1〜3・払戻は結果ページ（boatrace.jp）から取得した値で、Kファイル（別の公式ファイル）との突合は独立している。rank4〜6・進入は`kfile_sync`がKファイルから書いた値のため、Kとの突合は「同期が完了しているか・書き込みが壊れていないか」の確認で、取得元の誤りは検知できない（同じ元との比較）。

**分類（不一致に混ぜない）**

| 分類 | 内容 | 扱い |
|---|---|---|
| 照合不能 | Kが未取得（404）・会場が未展開（プレースホルダ）・Kに会場が無い | 別に数える。最後の照合まで再照合し、最後に通知する |
| 除外 | 確定中止（`cancellation_status='confirmed'`） | 除外した件数を報告。ただしKに結果があれば不整合（`cancelled_but_in_k`） |
| 同期待ち | rank4〜6・進入が全てNULL（`kfile_sync`の前） | 最後の照合の前は不一致に数えない。最後の照合で不一致にする |

**実行時刻**: JST 08:00・12:30・17:30（vercel.json。UTC 23:00・03:30・08:30）。`kfile_sync`（07:00・12:00 JST）の後。対象日は`07:50 JST`指定の日付の前日（D+1の朝に、Dを照合する）。最後の回（17:30 JST）で、同期待ちを不一致に数え、照合不能を通知して、対象日を処理済みにする。

**通知**: 不一致が1レース以上（`MISMATCH_ALERT_MIN_RACES`）で、レースID・項目・DB値・K値を`last_report`に上限30件で残し、`last_report.alerts`→`scrape-monitor`でSlackへ。通知は1回きり（`until`=30分後。`scrape-monitor`は5分ごとに評価し、同じキーを6時間再通知しない）。shadowは通知を出さず`would_alert`に記録する。`last_report.history`に、直近14日の要約（照合・一致・不一致・照合不能・同期待ち）を残す（継続監視・データ健全性レポートが読む）。

**Kファイルの共有**: ダウンロードは`kfile_sync`とは別の呼び出しのため、同じ日のKファイルを1日に1回、追加でダウンロードする（`www1.mbrace.or.jp`への1リクエスト/日）。関数の呼び出しをまたいで共有する手段（保管先）が無く、`kfile_sync`に保管を足すと、live並走中のジョブを変えることになる。`kfile_sync`は未同期のレースが無い日はダウンロードしないため、これが「同期が空振りの日も、照合は必ず動く」独立性にもなる。

**実データでの試験（読み取りのみ。実際のKファイルと本番DB）**

| 日 | 照合 | 結果 |
|---|---|---|
| 2026-09-15 | 156レース | 全て一致 |
| 2026-09-16 | 156レース | 全て一致 |
| 2026-09-18 | 180レース | 179一致。**1件の不一致**: 江戸川11Rの2連複が、Kは特払い70、DBはNULL（実際の欠落） |
| 2026-09-19 | 156レース | 全て一致 |
| 2026-09-20 | 168レース | 全て一致 |
| 2026-09-11 | 108レース | 一致。**36レースが`race_missing_in_db`**: Kは12会場、`races`は9会場で、住之江・浜名湖・戸田（いずれも第1日）の12レース×3会場が`races`に無い（原因は未調査） |

6日分（924レース）で、誤警告0・実際の不整合2種類。閾値1レースは、この実績で妥当と判断した（shadowで数日確認してからliveにする）。
