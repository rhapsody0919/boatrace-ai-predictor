...
| D9 | `race_notices_health`を、10分ごとに13会場分upsertする（内容の変化が無くても） | コード確認。**2026-09-20に、変更のある行のみに修正（T4b-11-1）。`last_checked_at`は「最後に書いた時刻」になり、最終確認は`scrape_job_state.last_success_at`** | A5 | 変更の無い行は書かない方針（WS8(b)）に反する |
...
| G4 | 特記事項（`race_special_notes`）が0件。ログは「0件新規保存」が続く。正常（通知が無い）か、パースの失敗かが不明 | 確認済み（0件）。**原因は、2026-09-20に確認: 通知が無いだけで、パースの失敗ではない**（tasks.md T4b-11-3。既知の実例9行を9件解析、2026年の実ページ8件は全て通知なし）。ただし、一意索引が同日・同内容の別選手の通知を1件に潰す（2017年の実例で9件中2件）ことを発見 | A5 | WS5 |
