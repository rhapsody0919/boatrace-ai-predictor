...
- 外部リクエスト: 通常日 108回/日（開催場一覧）。`shadow`は書き込まないため、候補が減らず、告知のある日は、毎回、候補の全レース（日全体順延の2会場で24件）を再取得する（約100起動/日）。`shadow`は順延・中止の発生日の検証に限り、確認後に`live`へ移す（`live`では、確定したレースは候補から外れる）。順延日は、確定までの候補レース×1回（確定後は取得しない）。反映待ちが長引くレースは、10分ごとに1回再確認する
...
2. `scrape_job_state`の`race_status`を`shadow`にする（DBの更新。承認後）。順延・中止が起きた日に、`last_report`を確認する:
   ...
   - `wouldConfirm`（打ち切らない。`notOnPage`・`errors`・`contradictions`は最大24件）が、その後に既存基盤（90分ルール）が確定したレースと一致する（`select race_id from races where race_date = '…' and cancellation_status = 'confirmed'`と突き合わせる）
