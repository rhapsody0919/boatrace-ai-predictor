...
### M-7. 朝の最初のレースの、発走60分前のオッズの未公開（想定内。tasks.md T4b-23、BOA-386）

**背景（2026-09-22の実測）**: 2026-09-22 07:35 JST、`scrape-monitor`が「`odds:2026-09-22-10-01:-60`（三国1R、発走08:32）が単勝を解析できず期限切れ（試行3回、`outcome=no_values`）」と通知した。旧基盤（GitHub）もこのレースの行を持っておらず、新旧で同じ結果（取得の失敗ではなく、公式の公開が窓の後に始まった）。同じレースは、その後 -30 も取れず、-15（08:17）で取れた。各会場の第1レースは、朝の開催（08時台の発走）が多く、発売・公開が発走60分前の窓の後に始まることがある。毎朝、同種の警告が出ると警告疲れになり、オッズのGitHub側停止（9/23 06:30）の朝に、最初のレースの警告（07:30頃）が、停止の失敗と紛らわしくなる。

**実測（読み取りのみ。直近7日 9/15〜9/21、`data-health-report.js`の窓内取得率の定義。結果確定済み・中止除外）**:

| 対象 | 件数 | 窓内（60分前±3分） | 窓内取得率 |
|---|---:|---:|---:|
| 第1レースの -60 | 94 | 76 | 80.9% |
| 第1レース以外の -60 | 1,023 | 917 | 89.6% |
| 第1レースの -60 のうち、窓内に取れず、後続の窓（-30〜0）に取れたもの（想定内の未公開） | 18 | - | - |
| 第1レースの -60 のうち、窓内にも後続の窓にも取れていないもの | 0 | - | - |

`-60`の全体の窓内取得率は、除外なし 88.9%（993/1,117）→ 想定内の未公開を分母から外して 90.4%（993/1,099）。日別（9/15〜21）の想定内の未公開は 2・2・2・3・2・3・4 件（1日2〜4件）。

**判定（定義の正本: `scripts/lib/scrapeJobs/expectedUnpublished.js`。監視とレポートが共有する）**: 次を全て満たすスロットだけを「想定内の未公開」とする。

1. ジョブ`odds`の`offset_min = -60`
2. その会場・その日の第1レース（`races`の最小のレース番号。`race_id`の末尾には依存しない。確定中止のレースも含めて最小を決める）
3. 最後の結果が`no_values`（試行が1回以上。未実行・`error`・`partial`は対象外）
4. 同じレースの後続の窓（-30・-15・-10・-5・0）のどれかが、`done`で、`outcome`が`ok`または`skipped_have_data`（後で公開されたことの確認。`shadow`・`cancelled_race`は確認に使わない）

| 後続の窓の状態 | 判定 | 警告（`expired`） | 窓内取得率（監視） |
|---|---|---|---|
| どれかが取れた | 確認済み（`confirmed`） | 出さない | 分母から外し、「未公開(想定内)」に数える |
| 取れたものは無く、期限+許容幅（3分）に達していない窓がある | 保留（`deferred`） | 出さない（保留） | 分母に入れず、「判定保留」に数える |
| 全ての窓が過ぎて、どれも取れていない・後続の窓が予定表に無い | 対象外 | 従来どおり出す | 従来どおり欠落（`expired`）として数える |

- 保留の間に警告を出さないのは、後続の窓で取れる前に警告するのを避けるため。**本当に取れていない第1レースの警告は、後続の窓が全て過ぎたとき（最大で発走の3分後）まで遅れる**（従来は、-60の期限+3分＝発走の57分前）。`error`・未実行（`attempts=0`）は対象外のため、これらの警告は遅れない
- 保留を分母から外すのは、窓内取得率の警告（当日の集計。5分ごと）が、後続の窓で取れる前に、欠落として数えて出るのを避けるため。日次サマリー（翌日01:00頃）の時点では、全ての窓が過ぎているため、保留は残らない
- 分母から外した件数は、必ず出す（黙って除外しない）: 日次サマリーの`odds`の行の「未公開(想定内・分母から除外) 前日 N件・直近7日 N件」（0件でも出す）、窓内取得率の警告の文面、`data-health-report.js`の「第1レースの60分前の窓」の表
- `data-health-report.js`は、`race_odds.captured_at`で同じ条件を判定する（窓内に取れておらず、後続の窓（-30・-15・-10・-5・0分前の±3分）のどれかに取れている）。閾値判定（`belowThreshold`）は除外後の値（`rateAdjusted`）で行い、除外なしの値・除外件数も併記する

**変更前後の警告（2026-09-22の実データ。本番の読み取りに、新しい判定を当てた）**: 08:19 JST の時点の`odds`の`expired`は5件（三国1Rの -60・-30、徳山1Rの -60、三国2Rの -60、徳山2Rの -60）。新しい判定では3件（三国1Rの -30、三国2Rの -60、徳山2Rの -60）。元の警告の時刻（07:35 JST。三国1Rの -60 の1件）では、-30 が未到来のため、保留（警告が出ない）になる（-60 の期限切れ時点で -30 以降が pending のスロットを再現して確認）。

**未確認事項・残る警告（別の判断が要る）**:

- **第1レース以外の -60 にも、同じ未公開が出る**。朝の開催（発走が10時前）の会場では、第2レースの -60 も窓の後に公開が始まる（9/15〜9/21の実測: 発走が10時前のレース R1は15件中15件、R2は15件中14件が -60 の窓内に取れず、R3は15件中2件）。9/22の三国2R・徳山2Rが該当する。第1レースだけの除外では、これらの警告は残る。対象を「発走時刻が朝の開催の枠（例: 10時前）」「第1・第2レース」などに広げるかは、公開が始まる時刻の実測（会場ごとの発売開始時刻）が要る。`expectedUnpublished.js`の定義を変えれば、監視・レポートの両方に反映される
- **第1レースの -30 にも未公開が残る**（9/15〜9/21: 94件中10件が -30 の窓内に取れなかった。9/22の三国1Rの -30 が該当）。-30 の警告は、この変更の対象外（要件は -60 のみ）。同じ扱いに広げるなら、対象の窓を追加する（後続の窓で確認する条件は同じ）
- 第1レース以外の -60 の窓内取得率は 89.6% のまま（上表）。この補正だけでは、-60 は完了の定義B（98%）に届かない。未公開以外の取りこぼし（GitHub Actionsの実行間隔による、窓に実行が入らない等）の内訳は、別に確認が要る

### M-8. `-60`の窓の基準の見直し: 発売開始の遅れを分母から除外し、検知の遅れで評価する（tasks.md T4b-24、BOA-386続き。2026-09-22ユーザー承認）

**背景**: M-7（T4b-23）は、後続の窓（`-30`〜`0`）で公開が確認できた第1レースの`-60`だけを、窓内取得率の分母から除外した。しかし未確認事項に残したとおり、(1) 第1レース以外（朝の開催会場のR2）にも同じ未公開が出る、(2) 分母から外すだけで、その除外したレースが「実際にはどれだけ遅れて検知できたか」を計測していなかった。オッズは取り直せないデータのため、`-60`の窓のリクエスト方式自体は変えず、以下の2点だけを変更する。

**新基準（`.claude/rules/data-acquisition.md`のB、2026-09-22改訂）**:

1. 窓内取得率の分母からの除外は、**全レース**（第1レースに限らない）に広げる。除外の条件は変えない: その`-60`スロットが未公開（`no_values`）で、同じレースの後続の窓（`-30`〜`0`）のどれかが取得できている（後で発売が始まった証拠がある）場合だけ。除外件数は必ず別に出す（黙って除外しない）
2. 除外したレースは、別指標「**発売開始の検知の遅れ**」（発売開始後の最初の取得の時刻と、その直前の未公開の試行の時刻の差）で評価する。基準は5分以内の割合98%以上
3. 上記2を満たすため、`-60`の窓は、未公開の間、発売開始まで（最大で`-30`の窓が始まるまで＝許容幅30分）、1分間隔で再試行を続ける（レジストリ`odds.graceMinByOffset: {-60: 30}`）。他の窓（`-30`〜`0`）・他のジョブ・他のデータ種別には適用しない

**実現方式（DDLの要否）**: 予定表の取得（`claim_scrape_slots`。マイグレーション075のRPC）は、許容幅をジョブ単位の引数`p_grace_min`ひとつで受け、期限切れ化・取得対象の判定の両方に使う。`-60`だけ許容幅を変えるには、offsetごとの許容幅の上書きが要るため、既存の関数は変えず、新関数`claim_scrape_slots_by_offset`（マイグレーション`092_claim_scrape_slots_by_offset.sql`）を追加した。既存の`claim_scrape_slots`の本体をそのままコピーし、`p_grace_by_offset`（JSONB、例`{"-60": 30}`）で該当offsetの許容幅だけ上書きできるようにしたもの。`p_grace_by_offset`が空のときは、既存の`claim_scrape_slots`と同じ結果になることを、PGliteの差分テスト（`verify:scrape-slots-sql`）で確認済み。権限（service_roleのみEXECUTE）も既存と同じ。

コード（`store.js`）は、上書きのあるジョブ（`odds`）だけ新関数を呼び、関数が無い（`PGRST202`。092未適用）ときだけ既存の`claim_scrape_slots`へ自動フォールバックする（延長は効かないが、オッズの取得自体は止まらない）。フォールバックしたことは`scrape_job_state.last_report.alerts`に残り、監視（`report:odds:claim_fallback`）が通知するため、092が未適用のまま延長が効いていない状態を見落とさない。**このため、092の適用より先にコードをマージ・デプロイしても安全**（適用後、次の`odds`の実行から自動的に新関数を使い始める。コードの再デプロイは不要）。

**追加リクエストの見積り**: `-60`が未公開のレースだけ、未公開の間、1分間隔で単勝ページ1枚だけを再試行する（`winFirst`。全通り5ページではなく1ページに絞る）。直近7日（9/16〜9/21）の実測で、`-60`が未公開（窓内に取れず）のレースは合計120件（6日間、1日あたり約20件）。延長の許容幅は最大27分（`-60`の窓の許容幅3分〜`-30`の窓の開始である30分の間）のため、1レースあたり最大27回、1日あたり最大約540リクエストの増加見込み（20件×27回）。取得先（boatrace.jp）は共有のサーキットブレーカーで保護されており、失敗率が悪化していないかは、切り替え後に確認する（T4b-24-3）。

**変更前後の指標の実測（2026-09-22、本番の読み取りのみ。新定義を実データに当てた結果）**:

| 対象 | 件数 |
|---|---:|
| 直近7日（9/16〜9/21）の`-60`の未公開（窓内に取れず。全レース） | 120件（第1レース16件・第1レース以外104件） |
| うち、後続の窓で確認できた（分母から除外） | 115件 |
| うち、後続の窓にも取れていない（本当の欠落。従来どおり警告） | 5件 |

発売開始の検知の遅れ（延長導入前。後続の窓での取得時刻－窓が閉じた時刻の近似値。`data-health-report.js`の`detectionLag`クエリ）: p50 27.6分・p95 47.4分・最大52.6分、5分以内の割合 **0%（0/115）**。延長を導入しない現状は、この基準（98%以上）から大きく外れている。延長（1分間隔の再試行）導入後は、この遅れがおおむね1〜2分程度に縮む見込み（`verify-scrape-slots-sql.js`のPGliteシナリオで、延長中は毎分再試行を継続でき、`-30`の窓の開始で自動的に打ち切られることを確認済み）。

**検証**: `npm run verify:scrape-jobs`（レジストリの`graceMinByOffset`の整合性検査、`claimSlots`の新関数呼び出し・フォールバック）・`verify:scrape-monitor`（`isExtensionSuccess`・`detectionLagOf`・`summarizeDetectionLags`・`evaluateDetectionLag`・日次サマリーの検知の遅れの行）・`verify:scrape-odds-job`（`winFirst`が未公開の間だけ単勝1ページに絞り、公開済み・他の窓・初回の試行では従来どおり全ページを取ることを確認）・`verify:data-health-report`（`detectionLag`クエリ・集計・Markdown・警告）・`verify:scrape-slots-sql`（PGliteで092を適用し、①`p_grace_by_offset='{}'`のとき既存の`claim_scrape_slots`と同じ予定表・同じ時刻で同じ結果になる差分テスト（期限切れ・許容幅内・リース・奪取の複数シナリオ）、②`-60`の上書きだけが延長され、`-30`・`-15`は変わらないこと、③延長後の許容幅を過ぎればリース切れで`expired`になる（無限に延長しない）こと、④権限がservice_roleのみであることを確認）。全て`npm run build`とあわせてグリーン。

**未確認事項**:

- 予定表（`scrape_slots`）に、offsetごとの許容幅の値そのものは保存しない（都度、レジストリの`graceMinByOffset`から計算する）ため、レジストリの値を変えれば、次の`claim`から即座に反映される（DDL不要）反面、過去のスロットが「どの許容幅で判定されたか」を後から追跡できない。監視・レポートは、現在のレジストリの値で常に再計算するため、この検証で確認した実測値は、延長導入後に再実測して確定する
- `data-health-report.js`の`detectionLag`はSQL（`race_odds.captured_at`）のみで近似する値であり、予定表の実際の試行時刻（`first_attempt_at`・`last_attempt_at`）は読まない設計。監視（`monitor.js`）側は`scrape_slots`の実際の試行時刻で正確に計測するため、日次の傾向はレポート、異常検知は監視、という役割分担にした。両者の値が大きくずれる場合は、近似の前提（後続の窓の取得時刻が、そのまま最初の公開検知に対応する）が崩れている可能性があり、要追加調査
- `verify:migration-rls`は関数（`FUNCTION`）のREVOKE/GRANTを静的検査の対象にしていない（テーブルのRLS・GRANTが主眼）。今回は`verify:scrape-slots-sql`のPGlite上で`has_function_privilege`を使った機能検証（実際に anon・authenticated が実行できないことを確認）で代替した。089（`data_health_functions.sql`）のような関数専用の静的検査（`verify:data-health-job`の方式）を、関数一般に広げるかは、必要になったら検討する
