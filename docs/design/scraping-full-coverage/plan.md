...
- **変化頻度**: 能力指数・フライング回数・出遅れ回数は集計期間（前期5/1-10/31・後期11/1-4/30）ごと、つまり**半年に1回**しか変化しない。生年月日・血液型等は実質不変。新規登録選手の追加は月に数件程度の低頻度
- **許容鮮度**: 数週間〜1ヶ月古くても実害は薄い（能力指数等をリアルタイム性が必要な用途に使う予定は無い）
- **可用性ウィンドウ**: 無し（常時閲覧可能）
- **結論**: **週次は過剰**（半年に1回しか変わらないデータを毎週再取得するのは無駄働き）。全選手（約1,600人）を毎週巡回するコストに見合わない。**月次を基本とし、集計期間切り替わり（5/1・11/1）直後の1〜2週間だけ週次にブースト**する

新規GitHub Actionsワークフロー、**月次実行**（例: 毎月1日）。5/1・11/1を含む月は、その月の第1〜2週のみ追加で週次実行する（cron式を月によって出し分けるか、常時月4回実行して差分を見るかは`/step4`で判断）。低頻度・静的データ（T5）でありBOA-313の対象外。
...
| BOA-294 モーター/ボートランキング・前検ランキング・出目データ・高配当ランキング | 前検ランキングは当日の前検後に確定、他は日次〜週次で緩やかに変化 | 当日〜翌日程度 | 前検ランキングのみ開催当日の前検時間帯に限定される可能性、Phase 6a調査で要確認 | **日次を基本**とし、前検ランキングのみPhase 6a調査結果次第でT1〜T2（開催当日のみ高頻度）に個別対応 |
| BOA-294 水面特性（会場の物理特性） | 実質不変（会場改修等がない限り変化しない） | 数ヶ月〜1年古くても問題ない | 無し | **年1回、または新規会場追加時のみ**（ルーチンポーリングにしない） |
| BOA-295 潮汐（潮位・満干潮時刻） | 天文計算に基づき将来分まで確定的に決まる | 決定論的データのため「鮮度」という概念自体が無い | 無し | **年1回のバルク取得**（1年分をまとめて取得、または将来的に自社での天文計算への切替も検討） |
...
| FR-2 期別成績・FR-5 profiles自動化 | T5 | **月次＋5/1・11/1直後2週間は週次** | 新規GitHub Actions | 半年に1回しか変化しないため週次固定は過剰と判断 |
...
| FR-4 オッズ全券種 | T1 | **60/30/15/10/5/0分前**（0分追加）、**全通り捕捉は10/5/0分前の3窓**に拡大 | 既存`scrape-odds.js`拡張→BOA-313 Phase 3で正式移行 | ADR-0057、依存あり |
| FR-6 進入コース別選手成績(BOA-293) | T4 | **日次** | 会場別GitHub Actions | - |
| FR-6 モーター/ボートランキング等(BOA-294一部) | T4〜T2 | **日次基本、前検ランキングのみ要個別確認** | 会場別GitHub Actions | Phase 6a調査後に確定 |
| FR-6 水面特性(BOA-294一部) | T5 | **年1回** | 会場別GitHub Actions | ルーチンポーリング対象外 |
| FR-6 潮汐(BOA-295) | T5 | **年1回バルク取得** | 会場別GitHub Actions | 決定論的データ、自社計算への切替も将来検討 |
