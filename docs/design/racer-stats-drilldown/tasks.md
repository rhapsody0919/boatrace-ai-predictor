...
- [x] **Task 8: `aggregateRacerCrossStats`を新規実装する**
  ...
  - 純粋関数（I/O無し）
  - セルフレビューで`aggregateRacerVenueBoatStats`との勝敗集計ロジック重複を指摘され、`tallyWinPlaceShow`共通ヘルパーに切り出し済み

- [x] **Task 9: 成績ブロックを4分岐で実装する**
  ...
  - セルフレビューで4テーブルのJSX重複を指摘され、`StatBreakdownTable`共通コンポーネントに切り出し済み

- [x] **Task 10: 一覧テーブルの行クリックでフィルタに反映する**
...
- [x] **Task 11: 決まり手・推移・レース一覧を常時表示化する（タブ廃止）**
...
- [x] **Task 12: 実データでの動作確認**
  ...
  - `npm run build` / `npm run test:e2e`（908件パス）
  - デスクトップ/モバイル(375px)・ライト/ダークテーマ、racerId変更時のフィルタリセットも確認済み
