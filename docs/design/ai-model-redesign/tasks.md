...
- **BOA-176実装完了**: 「複勝オッズは取得してほしい」と承認。調査の結果、単勝オッズと同一ページ（`oddstf`）に複勝オッズも掲載されており新規スクレイピングページ不要と判明。`scrapePlaceOdds()`を追加、`docs/db-migration/032_add_place_odds_to_race_odds.sql`。**ユーザーがSupabase Dashboardで手動適用済み（2026-08-14）**
- **残っていた旧モデル残骸の除去（BOA-173関連の見落とし対応）**: `RaceResult.jsx`（「レース結果」パネル、配当金額付き）にも単勝的中/3連複的中/3連単的中の表示が残っていたのを発見・修正（複勝的中+展開予測的中の2種類に統一）。`supabaseDataService.js`のデッドコード（`isHitWin`/`isHitPlace`、どこからも未参照）を削除。FAQ.jsx/Contact.jsx/About.jsxが既に廃止された3モデル切替機能の説明・古い精度実績値を表示していたため、unifiedモデルの実測値（複勝的中91.9%・回収146.0%、展開予測的中80.0%、過去4,580レース）に更新
- **BOA-178新規作成**: `/races`一覧ページ（RaceHistory.jsx）も同じ問題を抱えているが、`race_history_cache`テーブル＋バックエンドバッチの改修が必要な規模のためチケット化
...
- [BOA-178](https://linear.app/boat-ai/issue/BOA-178) `/races`一覧ページをunifiedモデルに合わせて根本再設計
