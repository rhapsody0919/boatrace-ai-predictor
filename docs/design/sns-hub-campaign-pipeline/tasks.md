...
- [x] **21. 結果発表に「なぜ当たった/外れたか」の振り返りを追加**（2026-09-09、ユーザー要望）: サイトのレース詳細ページ「データで振り返る」（`src/components/race/raceIndicators.jsx`・`RaceReview.jsx`）と同じ方法論（指標のレース内順位が上位2/下位2なら強気/弱気とみなし、実際の着順と突き合わせて整合/不整合を判定する）を、企画が既に持つ実データ（全国勝率・モーター2連率・展開予測）だけで簡易的に再現した。7指標フル移植（サイト本実装と完全に同じ精度）はNode側でのデータ取得ロジック新設が必要で工数に見合わないと判断し、ユーザー合意の上でこの簡易版にした。`scripts/lib/campaignResultRetrospective.js`（新設）の`buildResultRetrospective()`が本命艇・実際の1着艇・展開予測の3点について✅整合/⚠️不整合の文を生成し、`campaign-backfill-results.js`が結果発表ネタの`topic_text`に埋め込む。実際のびわこ2R結果（2-1-6、本命3号艇は全国勝率・モーター2連率とも1位も着外）で検証済み。`sns-pipeline-x.md`に、この材料を自然な日本語のプローズに書き直し「✅データと整合した点」「⚠️データと違った点」の2見出しで構造化する指示と実例を追加した
