...
- [x] **C5. Chugoku地域の4会場を追加**（[BOA-146](https://linear.app/boat-ai/issue/BOA-146)）
  児島・宮島・徳山・下関を同水準で追加
- [x] **C6. Kyushu地域の残り4会場を追加**（[BOA-147](https://linear.app/boat-ai/issue/BOA-147)）
  若松・芦屋・唐津・大村を同水準で追加
...
- [x] **D2. 月次計測の運用開始**（[BOA-149](https://linear.app/boat-ai/issue/BOA-149)）
  `i18n-demand-report.js` のベースラインを記録済み（`data/analysis/i18n-demand/report-2026-07-27.json`、全24会場公開後）。3ヶ月後（2026-10-27頃、spec.mdの数値目標判定時期）の比較基準とする。`search-console-report.js` は `SEARCH_CONSOLE_SITE_URL` 未設定・Search Console側の権限付与が未完了のため、ユーザー側のセットアップ（`docs/operation/search-console-report.md`）完了後に別途ベースラインを記録する
