...
| 8 | **朝のページの定刻性** | GitHub Actions の定時実行は実測で2.5〜4.5時間遅れる（§2.4）。JST 05:30 を狙っても公開は 08:00〜12:30 になる。ADR-0066 は「取得済みデータのDB内集計は GitHub Actions のまま」と定めているが、朝に見せることが価値の中心のこのページでは前提が成立しない。Vercel Cron へ移す（定刻性は上がるが ADR-0066 の方針変更）か、遅延を受け入れて文言で吸収するかを決める必要がある。2026-09-25 時点では後者（文言のみ修正）で暫定運用 |
