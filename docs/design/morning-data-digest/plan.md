...
flowchart TB
    ...
    subgraph B2["早朝バッチ JST 05:30 / 06:30 / 08:00（GitHub Actions）"]
...
**改訂（2026-09-24、ユーザー判断）**: Wilson95%下限は**画面に出さない**ことにした。「信頼下限」→「控えめに見て」と言い換えてもなお、%の数値が1つ増えることに変わりはなく、「どれを見ればいいのか」が分からなくなるため。母数の少なさは `is_small_sample`（母数20未満）による「⚠ 母数が少なく振れ幅が大きい」という文だけで伝える。値自体は `morning_digest_rows.metric_wilson_lower` に引き続き保存する（列は消さない）。

## 2.4 本番で発覚した実行順序の破綻（2026-09-25）

**定時実行では一度も書けていなかった。** 本番の `/today` が 9月25日の朝10時を過ぎても「本日ぶんはまだ公開されていません」のままだった件の調査結果。

### 根本原因: 集計が材料より先に走る

| ジョブ | 実行基盤 | 予定時刻(JST) | 役割 |
|---|---|---|---|
| `kfile-sync` | Vercel Cron | **07:00** / 12:00 | Kファイルから `race_results.actual_course_1〜6`（進入コース）を埋める |
| `aggregate-racer-course-technique-stats` | GitHub Actions | **01:10** | 進入コース別の集計。`window_end` を進める |
| `generate-morning-digest` | GitHub Actions | 05:30 / 06:30 / 08:00 | ダイジェスト生成 |

集計の材料である `actual_course_1〜6` は **Kファイル同期が当日 JST 07:00 に書く**（Kファイル自体が「開催日の夜〜翌日」公開のため）。集計は JST 01:10 で6時間早く、**前日ぶんの進入コースをまだ見られない**。したがって `window_end` は構造上ぜったいに「前々日」までしか進まない。

ところが完全性チェックは `window_end >= 前日` を要求していた。**満たせる日が存在しない。** 9/25 の3回の実行はすべて次のログを出して書かずに終わり、最終回で検証が失敗した。

```
完全性チェックを満たさないため、書き込まずに終了します:
  - racer_course_technique_stats.window_end が 2026-09-23 で古すぎます（2026-09-24 以降が必要）
```

**開発中に気づけなかった理由**: 9/22〜9/24 の3日ぶんはすべて 2026-09-24 の JST 13:56〜14:47 に手で流したもの（`morning_digest_days.generated_at` で確認）。その時刻はすでに当日の `kfile-sync`（07:00）の後で `window_end=2026-09-23` になっており、9/24 のダイジェストが要求する `>= 2026-09-23` をたまたま満たしていた。**定時実行の成功を一度も観測しないまま完了扱いにしていた。**

### 対処

完全性チェックの許容を `STATS_MAX_LAG_DAYS = 2`（前々日以降）にした。

- 2 は構造上の下限。1 では永久に満たせない
- 3 以上にすると「集計が何日も止まっている」という本来検知したい異常を見逃す
- 影響: 朝の時点の選手集計に前日のレースが入らない。集計窓は約296日・1選手1コースあたり中央値26走なので、1日ぶん（最大1〜2走）の欠落は実質的に無視できる

### もう1つの発見: GitHub Actions のスケジュール遅延が常態

このリポジトリの定時ワークフローは、**2.5〜4.5時間遅れて起動するのが常態**だった（実測）。

| ワークフロー | 予定(UTC) | 実際(UTC) | 遅れ |
|---|---|---|---|
| `aggregate-racer-course-technique-stats` | 16:10 | 19:57 | +3h47m |
| `generate-morning-digest`（1回目） | 20:30 | 23:18 | +2h48m |
| `generate-morning-digest`（2回目） | 21:30 | 23:57 | +2h27m |
| `generate-morning-digest`（3回目） | 23:00 | 翌01:11 | +2h11m |
| `update-winning-technique-stats` | 15:35 | 18:16〜20:05（5日連続） | +2h41m〜+4h30m |

つまり JST 05:30 に生成する設計でも、**実際に公開されるのは JST 08:00〜12:30 になる**。画面の「毎朝5時半ごろに更新します」は実態と合っていないため、時刻を書かない文言に変えた。

### 対処2: 両ジョブを Vercel Cron へ移した（2026-09-25、ユーザー判断）

定刻性そのものは、GitHub Actions に居る限り直らない。ADR-0066 が集計を対象外にした理由は「長時間のCPU処理を含むため」だが、実測ではこの2ジョブに当てはまらなかった（集計27.0秒・生成5.5秒、重い処理はPostgres側）。そこで **両方とも Vercel Cron へ移した**（[ADR-0066 §改訂1](../../adr/0066-scraping-execution-consolidation-to-vercel.md)）。

| ジョブ | 移行後の時刻(JST) | エンドポイント |
|---|---|---|
| `racer_course_technique_stats` | **13:00** / 17:00（補足） | `api/cron/racer-course-technique-stats.js` |
| `morning_digest` | **05:30** / 06:30 / 08:00（補足） | `api/cron/morning-digest.js` |

集計を 01:10 → 13:00 にしたのは `kfile_sync`（07:00 / 12:00）より後にするため。これで `window_end` がその日のうちに「前日」まで進み、翌朝 05:30 の生成まで約16時間の余裕ができる。

旧ワークフローは `workflow_dispatch` のみ残した（手動復旧・バックフィル用）。有効化には `scrape_job_state` への行の投入が要る（マイグレーション **099**）。
...
- 実行: **JST 05:30 / 06:30 / 08:00**（`cron: '30 20 * * *'` / `'30 21 * * *'` / `'0 23 * * *'`）。2回目以降は前の回が完全な結果を書けていれば何もしない。3回目は、`races-init` が全滅した日に GitHub 側のフェイルセーフが **JST 07:00** から初期化する（`scripts/lib/racesInit/ghaSkip.js` の `FALLBACK_FROM_JST_HOUR`）ぶんを拾うためのもので、検証（`verify-morning-digest.js`）もここで行う。06:30 を最終回にすると、この日は生成されないまま失敗通知だけが出る
...
`races-init` cron（`*/2 20-23,0-14`）が全会場ぶんを揃えた後に unified 予測を生成する構造のため、両者はほぼ同時刻に揃う。**最も遅かった実測が 05:09** のため 05:30 を1回目とし、さらに遅れた日のために 06:30 の2回目、GHAフェイルセーフ（07:00開始）を待つ 08:00 の3回目を置く。
...
09-20 は最も早い行でも 08:36 で、08:00 の3回目でも間に合わない。必須にするとジョブが失敗する。
...
`race_entries.created_at` は **2026-09-20 より前が全件NULL**、`predictions` には `created_at` が無く `predicted_at` は日中の再生成で上書きされる。したがって「最初に投入された時刻」を事後に測れるのは **2026-09-24 の1日だけ**（entries 05:01〜05:09、unified 05:09）。**05:30 という値の根拠は n=1 である**ことを明記しておく。ADR-0066 が `morning-init` の Vercel 移行を予定しているため投入時刻は今後変わる。2回目以降（06:30 / 08:00）と `predictions` の任意化で、ずれても壊れないようにしてある。
...
| B2 が出走表の投入前に走る | 完全性チェック（§3.2）で書かずに終了。最終回（08:00）でも満たせなければジョブ失敗 |
...
| 8 | **朝のページの定刻性** | GitHub Actions の定時実行は実測で2.5〜4.5時間遅れる（§2.4）。JST 05:30 を狙っても公開は 08:00〜12:30 になる。ADR-0066 は「取得済みデータのDB内集計は GitHub Actions のまま」と定めているが、朝に見せることが価値の中心のこのページでは前提が成立しない。Vercel Cron へ移す（定刻性は上がるが ADR-0066 の方針変更）か、遅延を受け入れて文言で吸収するかを決める必要がある。**2026-09-25 に解決**: ユーザー判断で両ジョブを Vercel Cron へ移した（§2.4 対処2、ADR-0066 §改訂1）。画面の文言も時刻を約束しない形に変更済み |
