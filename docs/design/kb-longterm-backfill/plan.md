...
DDLは`docs/db-migration/072_kb_archive_tables.sql`（**本番へ未適用**。適用はユーザーの承認後）。外部キー・トリガーは付けない（投入コストを下げるため）。整合はCLIの検証（K/Bのレース集合・登録番号）で担保する。3表の関係は論理的なもの。
...
- 未検証: ユーザー端末での応答時間、`load --apply`（DDL未適用のため未実行）、2005〜2018年の日々の形式変化（サンプル外の日に未知の表記がありうる。`unparsed`・`extra_lines`と`anomalies.jsonl`で検知する設計）。
...
| 項目 | 保留中に | 理由 |
|---|---|---|
| `load --apply`・DDL 072の適用 | **保留**（ユーザー承認が前提） | 保存先の決定（A/B/C）、統合設計（WS3）との整合、本番DDL |
