...
「前づけ傾向」バッジは、フィルタの影響を受けない全国集計値を使うため、**フィルタの影響を受けない「概要カード」への追加**が自然（タブは無いため「どのタブに置くか」という論点自体が解消された）。

**受入基準**:
- [ ] 「概要カード」（`RacerPerformanceStats.jsx`、フィルタ非依存の全国勝率・平均STと同じ並び）に追加する
- [ ] `RacerGradeBadge.jsx`と同じトーン・配置パターンで実装されている
