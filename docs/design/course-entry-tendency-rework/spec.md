...
5. **会場別対応（2026-09-19）**: 選手によっては特定の会場でだけ動くため、会場別も見られるようにする。実データ（2025-09-19以降の出走）で粒度ごとの件数を確認した:
   ...
   | 会場×枠番（選手非依存） | 144 | 100% | 最小約1,496（欠場繰り上がり除外後） | ほぼ全部 |

   選手×会場×枠番は大半が薄いため、5走未満を非表示にすると8割の出走が空欄になる。そこで、走数を必ず併記し、5走未満は薄字の「参考」として出す。あわせて、選手非依存の**会場平均**（会場×枠番）を基準線として添える。会場差も大きい（6枠の枠外進入率は浜名湖17.6%〜びわこ5.7%、江戸川0.0%。内側の艇の欠場による繰り上がりは除外して集計）
...
- [x] 江戸川の値（6枠で0.0%）は実態（枠なり固定）と確認済みのため、他会場と同様に表示する（Task 4）
- [ ] 内側の艇の欠場でコースが繰り上がった出走は、会場平均・選手の進入傾向の両方から除外する
