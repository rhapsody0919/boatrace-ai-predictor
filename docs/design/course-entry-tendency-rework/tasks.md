...
- [x] **Task 4: 江戸川の値が実態かを確認する**（2026-09-19完了）
  - 結論: **江戸川の枠外進入率ほぼ0%は実態**。直近12ヶ月1,523レースで「動いた」ように見える24件は、すべて欠場による繰り上がり（真の前づけは0件）。江戸川は前づけ反則を厳しく取る枠なり固定の水面という一般的な説明とも整合する。江戸川の会場平均は非表示にせず表示する
  - 副産物の発見: 欠場による繰り上がり（内側の艇が欠場し、外側の艇のコースが1つ下がる）が枠外進入率を汚していた。全出走の0.4%だが、「動いた」出走の4.1%を占める。選手の意思による前づけではないため、**内側の艇が欠場した出走は選手の進入傾向・会場平均の両方から除外する**ことにした（`isShiftedByAbsence`、067のSQLの`shifted`）
  - 除外後の6枠の枠外進入率: 浜名湖17.6%〜びわこ5.7%、江戸川0.0%（除外前は浜名湖18.4%・びわこ6.5%・江戸川1.2%）。会場×枠番の最小走数は約1,496
