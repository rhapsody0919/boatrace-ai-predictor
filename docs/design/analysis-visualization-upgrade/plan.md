...
- **ST考察・直近10走・今節の日別成績（FR-1・FR-3）は+0本**。`getRacerScopedRaceStats` が既に取得しているデータから算出する
- **例外: 今節タブの「節内順位」は+3本**（2026-09-26追加、`getMeetScoreboard`）。得点率は単独では読めず（「3.67」だけでは準優に乗るか分からない）、節の全選手（約48名）の得点率が要る。`race_id` が固定長なので `like("__________-VV-__")` で会場を厳密に絞れ、**1節=432行を1クエリ**で引ける（実測550ms）。内訳は `race_entries` +1／`race_results` +1／`race_conditions` +1で、読み取りは約700行
  - **今節タブを開いたときだけ**発火する。キーはレース単位（`meet-scoreboard-v2-{raceId}`）で、6艇のどれを開いても使い回す
  - 基本情報タブの合計は `getRacerPeriodStats` +1 と合わせて**+4本**になり、非機能要件の「+3本以内」を1本超える。得点率を「節の中での位置」まで出さないとファンの判断材料にならない（2026-09-26ユーザー判断）ため、超過を受け入れた
