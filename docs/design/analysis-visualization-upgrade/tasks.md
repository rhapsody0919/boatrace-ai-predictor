...
> **進捗（2026-09-25、Claudeアカウント切替のため中断）**: T5-2・T5-2b は**実装とデータ検算まで完了、ブラウザでの自己検証が未実施**。T5-3・T5-4 は未着手。
> ブランチ `feature/phase-a-basic-info`（origin にpush済み、PR未作成）。
> 完了済みの検証: 純関数の実データ検算25項目・as-of期計算14項目がすべて通過、`npm run build` 成功、lintエラー0件。
> 未実施: Playwrightでの画面確認（ライト/ダーク・320/390px・4言語）、E2E追記、`npm run test:e2e`。

- [ ] **T5-2** バー展開に3つ目のタブ「条件別」を追加する（`rbit-expanded-tabs`）— **実装済み・画面確認待ち**
...
- [ ] **T5-2b** 「前期」は別枠にする（FR-4c）— **実装済み・画面確認待ち**
...
- [ ] **T5-3** F数バッジを `race_entries.f_count` に統一する（基本情報タブに新規追加＋ST考察カードを修正）
