...
- [x] **T6-1** 節内の日別の進入コース・ST・着順を表示する
  ...
  - **実装（2026-09-25）**: 基本情報タブのバー展開に4つ目のタブ「今節」を足した。表示は
    枠別情報タブの直近10走と同じ `RecentRunsBar` を再利用し、今節だけ日付ラベル（`showDateLabel`）を出す
  - **節の切り出しで2点補正した**（`buildMeetResults`）。(1) `groupIntoCurrentMeet` は日付の連続性しか
    見ないため**会場で先に絞る**（節は会場単位）。(2) **表示中のレースを終端の目印として足してから切り出す**。
    この関数は過去のレースでも呼ばれるので、目印が無いと「表示中のレースより後の節」を今節として返す
  - **検証**: 2026-09-24 桐生3R・登番4872で6走（9/20〜9/23）が出て、進入・着順・ST・ST順位の
    **24項目すべてがDB実値と一致**。節の初戦（2026-09-20 桐生5R）では空状態になり前節が混ざらない。
    E2Eを1件追加。4言語・ライト/ダーク・320px/430pxで表示確認
  - 副次: `RecentRunsBar` の帯が `flex: 1 1` で伸びており、6走だと「5走＋伸びた1走」になったため
    `flex: 0 1` に変えた（直近10走はちょうど5×2行なので見た目は変わらない）

- [x] **T6-2** 今節タブに「通常値との差」を出す（Phase A、2026-09-26）
  - 日別の生データだけでは「で、今日はどうなのか」が読めない。**ST（今節の平均 vs 今節を除く全期間）**と
    **展示タイム（今節の最初 → 直近）**を、表の前に1行ずつ出す。追加クエリ0本
  - 閾値: ST ±0.01秒、展示 ±0.03秒を超えたら「踏んでいる／慎重」「上向き／下向き」と言い切る
  - 検証: 2026-09-24桐生3R・登番4872で、今節ST 0.202 / 通常 0.184（154走）/ 展示 6.81→6.69 が
    すべてDB実値と一致
- [x] **T6-3** 今節の得点率を自前計算して出す（Phase B、2026-09-26）
  - **公式の得点率一覧（`race/pointrank`）は SG/G1 の4日目以降しか出ない**ことを実機で確認した
    （2026-09-26: 若松G1は表が出る／同日の桐生・戸田の一般戦は「※ データはありません。」）。
    `racer_series_points` が101行しか無いのは取得漏れではなくこれが理由。
    **ボートレース日和は一般戦でも得点率を出している**（桐生の一般戦で確認）ので、追随するには自前計算しかない
  - 計算規則を公式データで確定させた（`src/components/race/seriesPoints.js`）:
    1. 着順点（予選・一般）= 1着10 / 2着8 / 3着6 / 4着4 / 5着2 / 6着1
    2. **特別戦（ドリーム戦・選抜戦）は別配点** = 12 / 10 / 9 / 7 / 6 / 5
    3. **失格・落水・転覆は0点だが分母には含める**（実データでは着順に載らず `rank5`/`rank6` が null）
    4. **準優勝戦・優勝戦は算入しない**（公式の表も「◯日目12R終了時点」と明記）
  - **検証: 若松G1（2026-09-22〜）の公式得点率と、実装した純関数の出力を全選手照合して
    49名中49名で得点・得点率とも完全一致**
  - あわせて「今日この着順なら得点率がこうなる」の早見を出す（公式もレースごとに「得点率早見表」を持つ）
  - **やらなかったこと**: ①節内順位（節の全選手＝約45名分のデータが要るが、画面は出走6名分しか取っていない）
    ②「準優当確」の断定（準優進出者の最低得点率を12節で実測したところ **5.14〜7.75 とばらつき**、
    節の面子で動くため言い切れない）。③ボーダーの事前集計テーブルは新規マイグレーションが要るため別途
...
- [ ] **T7-1** `MotorWakuStatsGrid` に前検タイム・節時点の2連対率の列を追加する
  ...
  - **着手前の未決事項**: FR-4aは公式サイト由来の値の再表示にあたり、[ADR-0067](../../adr/0067-official-site-content-redisplay-policy.md) への追記と**ユーザー承認が前提**（spec.md「制約・前提」）。2026-09-25時点でADR-0067に `motor_pretest_stats` の記載は無い。手続きはピットレポート（BOA-379）と同じく「モック承認 → 画面実装・自己検証 → ADR-0067への記録」
