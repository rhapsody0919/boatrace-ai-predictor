...
E・J・K は [BOA-432](https://linear.app/boat-ai/issue/BOA-432) に起票済み。
残り4件は **Linearワークスペースが無料枠の課題数上限（`USAGE_LIMIT_EXCEEDED`）に達していて起票できなかった**ため、
ここに残す（枠が空き次第、起票する）。

- **F: 小標本フラグの閾値が指標ごとに合っていない**。`SMALL_SAMPLE_THRESHOLD` は全指標一律6だが、
  1着率は n=8 でも期待勝利数0.8で「0.0%」に意味が無い（実例: 2026-09-25桐生1R 3号艇の当地 0.0% n=8 に⚠が付かない）。
  指標ごとの閾値、または期待発生回数（n × 全体平均率）での判定に変える
- **G: 「波5cm以上」が会場横断の合算**。桐生の波5cmと戸田・江戸川の波5cmは水面の性質が違う。
  当地×波5cm以上の行、または条件別タブにも当地/全国のスコープを効かせる（関連: BOA-336・BOA-211）
- **H: 「前期」が絶対値だけで上がり調子か下がり調子か読めない**。今期の値は同じ画面のバーにあるので、
  前期との差分（例: 前期4.50 → 今期4.12）を出せる
- **I: `l_count`（出遅）を取得しているのに画面で使っていない**。L持ちもスタートの踏み方が変わるのでFと並べたい。
  あわせて「そのFが今節のものか」が分かると、今節初日にFを切った選手を見分けられる（関連: BOA-321）
