...
**追記（2026-09-19）: 欠落率の判定基準の修正と、展示タイムnull行の再取得**: Step 2〜4で見ていた欠落率は「`exhibition_data`に行が存在するか」の基準で、展示タイムがnullの行を欠落と数えていなかった。実際は、鳴門・丸亀・児島・江戸川等で公式ページが展示ST（スタート展示）を展示タイムより約2分先に公開するため、その間に取得すると`exhibition_time=null`・STのみの行が書かれ、`run()`が「行あり＝取得済み」としてスキップし展示タイムが補完されなかった（9/18は180レース中29レース＝16.1%、レース詳細の直前情報タブに「展示タイムはまだ取得できていません」が表示される）。この問題は移行前の9/10〜9/11にも発生しており、Vercel移行で生じたものではない（2分間隔cronでST先出しの窓に当たりやすくなった可能性はあるが未検証）。対応として、①取得済み判定を`exhibition_time`非nullに変更（`getRaceIdsWithExhibitionTime`）、②欠落率を展示タイム基準に変更、③`backfill-exhibition-time.js`で9/10〜9/19の109レースを補完した。上記の「9/15分は欠落率0.0%」等の実測値は旧基準のため、Phase 2着手判断には修正後の基準（展示タイム基準）での日次結果を使うこと。
...
2. ~~30秒タイムアウト対応の案A/Bどちらを採用するか~~ → **案Bで確定**。cron-job.orgのタイムアウト（30秒）と実際のスクレイピング所要時間は別々の関心事のため、`waitUntil()`（`@vercel/functions`）でレスポンスを即座に返し実処理をバックグラウンド継続する設計にした。理由: 案A（1回あたりの処理件数に上限を設ける）は、GitHub Actionsで起きたキュー詰まりを別の場所で再現するだけで根本解決にならない。1回あたりの処理レース数に人為的な上限は設けない
   ...
   - **追記（2026-09-14、PR #639セルフレビューで発見）**: 上記の副作用は実際にはタイムアウト関連の失敗に限らない。`waitUntil(runExhibition(...).catch(...))`は`runExhibition`が投げる**あらゆる**エラー（Supabase認証情報の失効で全upsertが即座に失敗する等、タイムアウトと無関係な即時失敗も含む）を`console.error`でログするのみで、cron-job.org側の失敗通知だけでなく、Vercel自身のエラー率監視（旧同期版が`res.status(500)`を返していたことで得られていた検知経路）も同時に失う。日次欠落率チェックは翌日以降にしか気づけないため、即時の全滅失敗が最大1日近く気づかれない可能性がある点を認識した上で採用する（Vercelの関数ログを都度確認する運用は前提にしない）

3. **追記（2026-09-14、PR #639セルフレビューで発見・修正済み）**: `scrape-exhibition-data.js`の`fetchExhibitionForRace()`がboatrace.jpへの`fetch()`にタイムアウトを設定していなかった。`maxDuration`を60→300秒に拡大したことで、ハングしたリクエスト1件がインスタンスを最大300秒占有しうる（従来比5倍）ようになり、かつ`waitUntil`の`.catch()`ではプラットフォームによる強制終了は捕捉できないため気づく手段が無かった。`scripts/lib/supabaseClient.js`が2026-08-13の同種の無期限ハング不具合対応で導入済みの`AbortController` + 15秒タイムアウトパターンを踏襲して修正した
...
1. cron-job.orgの実行間隔をPhase毎にどう設定するか（展示は30/15/10分前ウィンドウを狙うなら1〜2分間隔が望ましいが、既存のGitHub Actions実行と重複しすぎない配慮も要る）。**追記（2026-09-14、PR #639セルフレビューで発見）**: `waitUntil`化により応答が対象レース数に関わらず常に1〜2秒で返るため、cron-job.org側は前回invocationのバックグラウンド処理が完了したかを一切知らずに次の実行を予定通り発火する。`scrape-exhibition-data.js`の重複スキップ判定（`getExistingExhibitionRaceIds()`）は各`run()`開始時点で「書き込み済み」のrace_idしか見ないため、前回invocationがまだスクレイピング中（未書き込み）の間に次のinvocationが始まると、同じレースを重複スクレイピングしてしまう（boatrace.jpへの重複リクエスト増加・Supabaseへの重複書き込み、upsertのため実害は書き込み結果自体には出ないが、ボット判定リスクと無駄なActive CPU消費に繋がる）。対策として実行間隔を人為的にロックする仕組み（分散ロック等）を追加するのではなく、**cron-job.orgの間隔をバックグラウンド処理の実測完了時間（investigation.mdの実測ベースで数十秒程度）に対して十分余裕を持たせる**ことで運用上回避する方針とする（コードに複雑さを持ち込まない、案A不採用の理由と同じ判断軸）
   - **追記（2026-09-14、code-reviewスキルによる検証で発見）**: 「単純なskip-if-in-progressロックなら案A（件数キャップ）と違って待ち行列を作らないので安全」という反論があり得るが、これも無条件に安全とは言えない。ロックの解除条件（staleness timeout）を誤って長く設定すると、前回invocationが実際には完了しているのにロックが残り続け、次のtickをスキップしてしまい、結果的に30/15/10分前という狭いウィンドウ自体を取りこぼす——まさに今回の移行の発端となった「キューでの取りこぼし」と同型の障害を、キューでなくロックという別の形で再現しうる。ロック導入を検討する場合は、この失敗モードを上回るメリットがあるか、staleness timeoutの妥当性込みで別途検討すること
2. `scripts/daily/scrape-exhibition-data.js`の`run()`はベニュー全件のループが終わった後に1回だけSupabaseへupsertする設計（**未変更、既存の挙動**）。**追記（2026-09-14、PR #639セルフレビューで発見）**: 本PRで`maxDuration`を60→300秒に拡大し、1回あたりの処理レース数上限も撤廃したことで、この「一括アップサート」1回分の対象規模がこれまでより大きくなり得る。300秒到達でプラットフォームに強制終了された場合、その回に取得済みだった全データが書き込まれずに失われる。30/15/10分前の3段階リトライ機構により大半のケースは後続invocationで回収できるが、**最終（10分前）チェックポイントでの失敗だけは後続リトライが無いため、そのレースの展示データが恒久的に欠落しうる**。実測ベース（8レースで約55秒）では300秒到達は非常に稀と想定されるが、`run()`をベニュー単位の逐次書き込みに変更する等の対策は、既存の共有関数（GitHub Actions版と共用）への影響範囲が大きいため本PRのスコープ外とし、Step 2の並走期間中の欠落率監視で実際に問題化するかを見極めてから検討する
3. 並走期間の具体的な長さ（3日〜7日を目安としているが、レース開催日数の関係で調整の余地あり）
4. Phase 1完了後、Phase 2・3に進む前にユーザーへの中間報告を挟むかどうか
