...
cron-job.org側のタイムアウトは30秒（[investigation.md](../../proposal/scraping-serverless-migration/investigation.md)で確認済み）。Vercel Function自体は最大300秒（Hobby、Fluid Compute）まで動けるが、**cron-job.orgへの応答は30秒以内に返す必要がある**。実測ベース（展示データ8レースで約55秒、これは複数レースをまとめて処理した場合の数値）を踏まえ、以下のいずれかで対応する:

- **案A（推奨）**: 1回の呼び出しで処理するレース数を絞る（例: 発走ウィンドウの中でも直近のものだけ）ことで、実処理時間を確実に30秒未満に収める
- **案B**: 応答を先に返し（`res.status(202).end()`）、処理はバックグラウンドで継続する（Vercel Functionsの`waitUntil`相当の仕組みを使う。Fluid Compute環境での挙動を要確認）

実装時にA/Bどちらが適切か、実測しながら決定する。
...
1. Vercelの`api/`配下から`scripts/`配下のモジュールをimportする際のビルド・バンドル挙動（実機確認が必要）
2. cron-job.orgの実行間隔をPhase毎にどう設定するか（展示は30/15/10分前ウィンドウを狙うなら1〜2分間隔が望ましいが、既存のGitHub Actions実行と重複しすぎない配慮も要る）
3. 30秒タイムアウト対応の案A/Bどちらを採用するか（実測して決定）
4. 並走期間の具体的な長さ（3日〜7日を目安としているが、レース開催日数の関係で調整の余地あり）
5. Phase 1完了後、Phase 2・3に進む前にユーザーへの中間報告を挟むかどうか
