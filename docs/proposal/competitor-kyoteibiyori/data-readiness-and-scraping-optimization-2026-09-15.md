...
**訂正（2026-09-15）**: 当初「UI実装は軸①〜③すべてが安定してから」という一律の着手順序を提案したが、依存はタブごとに個別であり一律に待つ必要はないとユーザーに指摘され訂正した。BOA-313が実際にブロックするのは直前情報（BOA-304）のみで、他タブはそれぞれ別の理由（軸①の新規データ未取得 or 軸③の既知バグ）で止まっているに過ぎない。

| BOA-305配下のタブ | 実際のブロッカー | BOA-313（軸②）との関係 |
|---|---|---|
| モータ情報（BOA-308）・結果（BOA-312） | 無し（既存実装の導線整理・再配置が中心） | **無関係、着手可能** |
| 基本情報（BOA-306） | 無し（事故率・決まり手数等の集計元確認のみ） | **無関係、着手可能** |
| 枠別情報（BOA-307） | ③正確性: [BOA-257](https://linear.app/boat-ai/issue/BOA-257)（進入コースバグ）の影響を直接受ける | 無関係。BOA-257を先に直すべき |
| 今節成績（BOA-309） | ①网羅性: [BOA-291](https://linear.app/boat-ai/issue/BOA-291)（データ未取得）が前提 | 無関係。新規スクレイピングが先 |
| 直前情報（BOA-304） | ②信頼性: 展示ST・展示タイムの取得タイミングが不安定だとタブ新設しても「−」表示が頻発する | **BOA-313 Phase 1の切替完了を待つのが妥当** |
| オッズ検索（BOA-310）・オッズ一覧（BOA-311） | ①网羅性: [BOA-314](https://linear.app/boat-ai/issue/BOA-314)（3連複/2連単/2連複/拡連複が未取得）が前提 | 弱い関係（Phase 3がオッズ対象だが未着手。新規スクレイピングをどちらの基盤に乗せるかはBOA-314で要判断） |

## 推奨する着手順序（訂正版、2026-09-15）

1. **BOA-313（軸②）を待つのは直前情報タブ（BOA-304）のみ**。他タブをこれのために止める理由は無い
2. **今すぐ並行着手できるもの**: モータ情報（BOA-308）・結果（BOA-312）・基本情報（BOA-306）のUI実装、BOA-257（軸③、進入コースバグ）の修正、[BOA-291](https://linear.app/boat-ai/issue/BOA-291)（軸①、今節成績データ取得）・[BOA-314](https://linear.app/boat-ai/issue/BOA-314)（軸①、オッズ全券種データ取得）の新規スクレイピング実装
3. **枠別情報（BOA-307）はBOA-257の修正後**、**今節成績（BOA-309）はBOA-291の完了後**、**オッズ検索/一覧（BOA-310/311）はBOA-314の完了後**にそれぞれ着手する（タブごとに独立して進行可能）
4. **設計上の注意**: BOA-314（新規オッズスクレイピング）は、現行のGitHub Actionsオーケストレーター（BOA-313でまさに廃止しようとしている構造）に実装を追加するか、BOA-313が作るVercel Functions側の新パターンに合わせるかを先に決める。前者だと後で二重移行になる（BOA-314内に論点として記載済み）
