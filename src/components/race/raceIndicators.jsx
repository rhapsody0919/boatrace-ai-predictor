 ...
 * データ出走表（DataRaceTable）の行定義（ラベル・値レンダリング・最良艇・
 * 強弱シグナル・言語化）を集約する。
 * signal/itemText（言語化用の強弱判定・文言生成）は元々「データで振り返る」
 * （RaceReview、BOA-312で撤去済み）と共通化するために用意した仕組みだが、
 * DataRaceTable自体はこれらを使わない（shortLabel/renderのみ参照）。
 * 撤去時点で他に利用箇所が無いことを確認した上で、将来の再利用に備えて
 * 定義自体は残置している
 *
