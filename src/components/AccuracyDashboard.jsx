...
function AccuracyDashboard({ onRefresh, isRefreshing }) {
  ...
  const pageMeta = (
    <>
      ...
      <meta
        name="description"
        content="BoatAIのAI予測成績ダッシュボード。単勝・複勝・3連複・3連単の的中率と回収率をモデル別・会場別に公開中。"
      />
    ...
    return MODEL_KEYS.map((modelKey) => {
      ...
      return {
        ...
        trifectaRecoveryRate:
          thisMonth.actualRecovery?.trifecta?.recoveryRate || 0,
  ...
  const ModelComparisonTable = () => {
    ...
    return (
      <div className="model-comparison-section">
        <h3>
          📊 モデル間パフォーマンス比較（今月{dateRange ? `: ${dateRange}` : ""}
          ）
        </h3>
            ...
            <tbody>
              {modelComparison.map((model) => (
                <tr
                  key={model.key}
                  className={
                    model.key === selectedModel ? "selected-model" : ""
                  }
                >
                  <td className="model-name">{model.name}</td>
                  <td className="races-cell">
                    {model.races > 0 ? `${model.races}レース` : "-"}
                  </td>
                  <td className="hit-rate">
                    {model.races > 0 ? formatPercent(model.winHitRate) : "-"}
                  </td>
                  <td
                  ...
                  <td className="hit-rate">
                    {model.races > 0 ? formatPercent(model.placeHitRate) : "-"}
                  </td>
                  <td
                    className="recovery-rate"
                    style={{
                  ...
                  <td className="hit-rate">
                    {model.races > 0 ? formatPercent(model.trioHitRate) : "-"}
                  </td>
                  <td
                    className="recovery-rate"
                    style={{
  ...
  return (
    <>
      ...
      <div className="accuracy-dashboard">
        ...
        <div className="new-model-accuracy-notice">
          <h3>🆕 新AI予想モデルの実績</h3>
          <p>
            2026年8月13日にリリースした新AI予想モデル（複勝予想・展開予測・イン崩れ指数・3連単参考情報）の
            専用実績ページは準備中です。統計として意味のある件数が蓄積され次第、公開します。
          </p>
        </div>

        <div className="archive-section-header">
          <h3>📦 旧モデルの実績アーカイブ（2026年8月運用終了）</h3>
          <p>
            以下はスタンダード・本命狙い・穴狙いの3モデルの実績です。参考情報として公開を継続しています。
          </p>
        </div>
          ...
          <div className="no-data-message">
            ...
            {selectedModel !== "standard" && (
              <p className="model-info">
                💡
                過去のデータは全て「スタンダード」モデルとして記録されています。
                <br />
                ...
                <h3>
                  ...
                  {(() => {
                    ...
                    if (last7Days.length > 0) {
                      ...
                      const endDate =
                        last7Days[last7Days.length - 1].date.substring(5);
                      ...
                      {modelData.dailyHistory
                        ...
                        .map((day) => (
                          <tr key={day.date}>
                            ...
                            <td className="hit-rate">
                              {formatPercent(day.topPickHitRate)}
                            </td>
                            <td
                              className="recovery-rate"
                              style={{
                                color: getRecoveryColor(
                                  day.actualRecovery?.win?.recoveryRate || 0,
                                ),
                              }}
                            >
                              {day.actualRecovery?.win
                                ? formatPercent(
                                    day.actualRecovery.win.recoveryRate,
                                  )
                                : "-"}
                            </td>
                            <td className="hit-rate">
                              {formatPercent(day.topPickPlaceRate)}
                            </td>
                            <td
                              className="recovery-rate"
                              style={{
                                color: getRecoveryColor(
                                  day.actualRecovery?.place?.recoveryRate || 0,
                                ),
                              }}
                            >
                              {day.actualRecovery?.place
                                ? formatPercent(
                                    day.actualRecovery.place.recoveryRate,
                                  )
                                : "-"}
                            </td>
                            <td className="hit-rate">
                              {formatPercent(day.top3HitRate)}
                            </td>
                            <td
                              className="recovery-rate"
                              style={{
                                color: getRecoveryColor(
                                  day.actualRecovery?.trifecta?.recoveryRate ||
                                    0,
                                ),
                              }}
                            >
                              {day.actualRecovery?.trifecta
                                ? formatPercent(
                                    day.actualRecovery.trifecta.recoveryRate,
                                  )
                                : "-"}
                            ...
                            <td className="hit-rate">
                              {formatPercent(day.top3IncludedRate)}
                            </td>
                            <td
                              className="recovery-rate"
                              style={{
                                color: getRecoveryColor(
                                  day.actualRecovery?.trio?.recoveryRate || 0,
                                ),
                              }}
                            >
                              {day.actualRecovery?.trio
                                ? formatPercent(
                                    day.actualRecovery.trio.recoveryRate,
                                  )
                                : "-"}
                            </td>
                          </tr>
            ...
            <div className="accuracy-info">
              ...
              <div className="info-section">
                ...
                <ul>
                  <li>
                    <strong>単勝:</strong>{" "}
                    AI予想の本命（1位予想）が1着になった割合
                  </li>
                  <li>
                    <strong>複勝:</strong> AI予想の本命が2着以内に入った割合
                  </li>
                  <li>
                    <strong>3連複:</strong>{" "}
                    AI予想のトップ3が実際の1-2-3着を全て含んでいた割合（順序不問）
                  </li>
                  <li>
                    <strong>3連単:</strong>{" "}
                    AI予想のトップ3が実際の1-2-3着と順序も完全一致した割合
                  </li>
                </ul>
              </div>
              <div className="info-section">
