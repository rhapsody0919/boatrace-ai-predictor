 ...
 * マイクロフト予想 データ取得サービス
 *
 * マイクロフトは PyTorch（Transformer）のためブラウザ内推論ができない。
 * 日次バッチ（generate-mycroft-predictions.js、1日3回）が mycroft_predictions
 * テーブルに書いた予測を読み取って表示する（ワトソンと同じ配信方式）。
 * モデルのメタ情報は data/mycroft/model.json（学習時にコミット）から読む。
 */

import { supabase } from "./supabaseClient";
import model from "../../data/mycroft/model.json";
...
/** 会場コード → 会場名（「マイクロフトの記憶」の過去レース表示用） */
export function venueName(code) {
  return VENUE_NAMES[code] || `会場${code}`;
}

/** モデルのメタ情報（学習日・評価指標・ワトソン比較など）を返す */
export function getMycroftModelInfo() {
  return {
    ...
    vsWatson: model.vs_watson,
    ensemble: model.ensemble_with_watson,
 ...
 */
export async function getMycroftPredictions(date) {
  if (!supabase) return [];
  ...
  try {
    ...
    for (let i = 0; i < raceIds.length; i += 100) {
      const { data, error } = await supabase
        ...
        .select(
          "race_id, rank_order, win_probs, attention_evidence, model_trained_at, predicted_at",
        )
    ...
    return races
      ...
      .map((r) => {
        ...
        return {
          ...
          venueName: venueName(r.venue_code),
          raceNumber: r.race_number,
          startTime: r.start_time,
          rankOrder: p.rank_order,
          winProbs: p.win_probs,
          attentionEvidence: p.attention_evidence,
    ...
    console.error("マイクロフト予想の取得に失敗:", e);
    return [];
  }
}
