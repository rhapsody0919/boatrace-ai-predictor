...
- [ ] 的中レースページのモデル対応
  - **現状**: 的中率統計ページは3モデル対応済み
  - **残作業**: 的中レースページ（RaceResults.jsx）も3モデルに対応させる
...
### モデル追加のタイミング
- **現在**: 3モデルで運用中、各モデル15レースのデータ
- **判断基準**: 各モデルで最低100レース以上のデータが蓄積されてから
- **理由**: 統計的信頼性を確保するため（現在は信頼性警告を表示中）

### DB移行のタイミング
- **現在**: JSONファイルで約2,000レース（約9MB）
- **目安**: レース数が5,000～10,000件になったら検討
- **推奨時期**: 3～4ヶ月後
- **推奨DB**: Supabase（無料プラン500MB、PostgreSQL）

---

最終更新: 2025-12-19 13:45
