...
**大原則: 競合機能をそのまま模倣しない。** kyoteibiyori.com/vup/ の機能は着想・優先度判断の材料として使い、
実装案は boatAI の既存デザイン・コンポーネント・モダンなUI/UXで**より良いもの**に昇華させる前提で作成する
（競合の密なスプレッドシート型「一覧表」をそのまま再現するのではなく、既存ページ
（例: `OutcomeDistribution.jsx` 等の集計データ探索ページ）と同じ設計言語で作り直す、
予想根拠のフローに統合する等、boatAIらしい差別化ポイントを必ず盛り込む）。
...
3. **UI/UX実装案・技術的実装案を提示**（競合の見た目をそのまま再現しない。上記大原則に従う）
