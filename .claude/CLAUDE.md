...
### セッション開始時のXツイート下書き投稿確認（2026-08-23〜）
...
- **SNS投稿の自動化・自動承認は行わない**（Claude Code側の安全ルール上、送信を伴うアクションは1件ごとの明示的承認が必須で、「今後は自動で投稿してよい」という包括的な事前承認は成立しない。ブラウザ操作での投稿代行も自動モードの安全機構でブロックされる）。ユーザーが内容を確認し「投稿する」と答えたら、文面を提示するところまでを行い、実際の投稿操作はユーザーが手動で行う
...
- `data/analysis/x-posts/history.json`を確認し、本日まだ投稿していなければ、**このセッションの最初の応答で**「本日Xに動画を投稿しますか？」と自発的に確認する（ユーザーから話しかけられるのを待たない）
