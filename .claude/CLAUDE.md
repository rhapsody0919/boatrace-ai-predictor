...
- **1日の投稿本数は6本が上限**（2026-08-26追加、詳細は`docs/operation/tiktok-posting-operations.md`の「D. 1日の投稿本数上限」参照）。`history.json`の当日エントリが既に6件あれば、追加投稿は提案せず翌日に回す。同時に直近投稿の再生数が複数本連続で0になっていないか（シャドウバンの兆候）も確認し、兆候があれば新規投稿を見送りユーザーに報告する
