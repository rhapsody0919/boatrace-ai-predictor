...
会話が進むにつれて「あれもこれも」と関連する追加依頼が積み重なり、1つのセッションが当初の目的から離れた広い範囲を抱え込みやすい（2026-09-15、「AI運用の評価」という依頼から派生してMCP連携レビュー・Vercel本番障害対応・vite preview根本原因調査まで一気通貫で進んだ実例あり）。都度ユーザーに「別セッションにするか」を判断させず、新しい話が出るたびにClaude側が以下の基準で能動的に切り出し先を判断し、一言提案する。

**判断基準（3段階）**:
- **Agent（同一セッション内のサブエージェント）で足りる**: 今の会話の文脈に依存し、結果をすぐ元のタスクの意思決定に使う調査・検証（例: 独立した観点でのレビュー、探索作業のコンテキスト節約）。この場合は確認を挟まずそのまま実行してよい
- **`spawn_task`（バックグラウンドタスクとして切り出す）が適切**: 今の会話の文脈がなくても自己完結して着手でき、今は手が回らない別スコープの改善点（気づいたバグ、ついでに直したい既存の不備等）。プロンプトに必要な文脈を書き込めば足りる規模
- **別セッション（新しいworktree・新しい会話）に分けるべき**: 元の依頼と目的が全く異なる、規模が大きい、または今の会話のコンテキストを圧迫する見込みが強い場合

**確認の仕方**: 都度重く確認せず、「これは元の依頼と別スコープに見えるので、①このまま続ける②`spawn_task`で切り出す③別セッションに分ける、のどれがいいか」と一言添えるだけに留める（「確認のバッチ化」の原則と同じ発想）。

### 小さなスコープの作業中に「実は大きな設計判断が必要」と気づいた場合（2026-09-15〜）
今困っている問題は応急処置としてまず完了させることを優先する（止血が先。PR#624のVercel本番デプロイ障害対応で、根本原因の追究より先にビルドを通す応急処置を優先した判断と同じ発想）。大きな設計検討を理由に今の作業を止めない。

- 応急処置で終わらせず、完了報告に「これは応急処置であり、根本的な設計は別途検討が必要」と明記する
- 気づいた大きな設計課題は、その場で深追いせず、Linear起票（軽量チケット化）または`docs/proposal/`への記録で明示的に残す。ユーザーが「今すぐ大きい方をやりたい」と言えばそちらを優先する
- 再発性・影響範囲の兆候（同じ問題が2回目以降発生している、複数箇所に影響する、車輪の再発明になりそう等）が強い場合は、「これは個別修正ではなく`/step1-spec`からのSDDフローで俯瞰設計すべき規模かもしれない」と一言添えて判断を仰ぐ
