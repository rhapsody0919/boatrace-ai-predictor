...
### 無人時間帯の自律進行ルール
ユーザーが「寝るので」「離席するので」等、長時間反応できないことを伝えた上で「対応を進められるところまで進めて」と明示的に依頼した場合、以下の運用に切り替える。この運用は明示依頼があった時のみ適用し、通常セッションのデフォルト挙動ではない。

- 確認が必要な事項（マージ承認等）が発生してもその場でブロックしない。確認依頼はテキストで提示するだけに留め、待たずに次のタスクに着手する。確認事項が複数溜まったら、個別に聞かず1回のメッセージにまとめて提示する
- ただし以下は「進める」の対象外。判断がユーザー固有のもの、または結果を戻しにくいものは、作業を止めてユーザーの判断を仰ぐ（実行はしない）:
  - マージの実行そのもの（承認が無い限り実行しない。これは自律進行モードでも変わらない）
  - 破壊的なgit操作（force push、reset --hard等）
  - 本プロジェクト外（他リポジトリ・他プロジェクト）への変更の適用。関連する調査・下調べ（読み取り専用の確認）は続けてよいが、書き込みは伴わない
  - 事業判断・プロダクト判断が絡むもの（例: 2026-08-21、共有コマンドプラグインをhakumei-appにも適用するかの判断。hakumei-app側のコマンドが独自の承認フロー・デプロイ方式に分岐していたため、機械的な同期はプロダクト判断が必要と見て保留にした）
- 迷った場合は安全側に倒す。作業は止めずに続けるが、影響が大きい・後戻りしにくい行動は取らない
- 確認すべき点があるタスクで一定時間（目安30分）反応がなければ、状況を記録してそのタスクをいったん終了し、次のタスクに進む
