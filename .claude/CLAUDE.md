...
- **1日の投稿本数は1〜2本が上限**（2026-08-26追加、詳細は`docs/operation/tiktok-posting-operations.md`の「D. 1日の投稿本数上限」参照）。「毎日投稿」は「1日1本を継続する」という意味であり、複数本まとめて投稿してよいという意味ではない。`history.json`の当日エントリが既に1〜2件あれば、追加投稿は提案せず翌日に回す。同時に直近投稿の再生数が複数本連続で0になっていないか（シャドウバンの兆候）も確認し、兆候があれば新規投稿を見送りユーザーに報告する
