...
- 既存機能の修正・追加依頼で、影響範囲が1ファイルに閉じない、または挙動が変わる場合は、**実装方針を考える前に `spec-ambiguity-detector` へ依頼文をそのまま渡し**、決まっていない点を列挙させる。そのうえで「こう理解した」を一言で復唱し、ユーザーの認識とズレが無いか確認してから実装する
  - **自分で復唱するだけでは足りない**（既にこのルールがあった期間に、仕様・要件の取り違えによるfixが39件出ている。全fix 147件中の最多）。依頼を読んだ本人が復唱すると自分の解釈に固着する。依頼文だけを渡した別エージェントに判定させる構成で、SWE-bench Verified の情報欠落版で resolve rate が +14.6〜22.2pt 改善したという実測がある（[arXiv 2603.26233](https://arxiv.org/html/2603.26233)）
  - **確認が要る点が2つ以上あればタスクを分割する**。曖昧点が1個なら最高30%成功、2個で8%、3個でほぼゼロという実測がある（[arXiv 2607.00711](https://arxiv.org/html/2607.00711)）。「必要なら聞いて」と書くだけでは効かないことも同じ研究が示している
...
**長さによる判断は自動で促される。** セッションの編集回数が20回を超えると、PostToolUseフック（`.claude/hooks/track-session-progress.sh`）が現在の推定遵守率と下記の選択肢をその場で出す。以降10回ごとに再通知する。**指示遵守のオッズは関数を1つ生成するごとに約5.6%低下する**（Claude Code 1,650セッション・16,050観測、[arXiv 2605.10039](https://arxiv.org/abs/2605.10039)）。同じ研究で、CLAUDE.mdのサイズ・指示の位置・ファイル構成・矛盾の4変数はいずれも遵守率に有意差を与えないことが示されており（BF10で帰無仮説を積極支持）、**効くのは1セッションで生成する量を減らすことだけ**。
