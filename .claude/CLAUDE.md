...
- **低リスクカテゴリは復唱・事前確認を省略し、確認なしで着手してPR作成まで進めてよい**（2026-09-06合意）: typo修正、文言1箇所の変更、note/Xの投稿下書き生成、Linearチケット起票、ブログ記事の軽微な修正（誤字・数値更新・リンク修正等、記事の主張や構成を変えないもの）。この定義は「実装完了後の自動レビュー」のマージ確認バッチ化でも共通して使う
...
8. マージはユーザー承認後に実行（勝手にマージしない。本番masterへのマージ＝本番公開行為のため、1件ごとに明示的なyesを得てから実行する。これは以下のバッチ化の対象外＝実行そのものを省略することはない）
   ...
   - **低リスクカテゴリ（「実装前の要件確認」で定義したtypo修正・文言修正・下書き生成・Linear起票・ブログ軽微修正等）**: 個別に重いサマリーを都度提示せず、複数件たまったら1回のメッセージに「以下のPRをマージしてよいか」と一覧でまとめて確認する（2026-09-06合意）。ただしマージの実行自体は一覧内の各PRについて明示的な承認を得てから行う
...
- 判断が割れない軽微な確認（レビュー指摘のスキップ理由、下書き生成の完了報告、低リスクカテゴリの実装完了等）は都度ブロックせず、テキストで提示するだけに留めて次のタスクに進む。確認事項が複数溜まったら1回のメッセージにまとめて提示する
- **以下は従来通り、個別に明示的なyesを得るまで実行しない（バッチ化・省略の対象外）**: マージの実行そのもの、SNS/ブログ等への投稿の最終送信、破壊的なgit操作（force push・reset --hard等）、購入・決済、他プロジェクトへの書き込み変更、事業判断・プロダクト判断が絡むもの。これらは1件ごとに明示的な承認を得てから実行する（複数件を1つのメッセージにまとめて確認を求めるのは可、実行の省略は不可）
