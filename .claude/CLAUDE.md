...
- **低リスクカテゴリは復唱・事前確認を省略し、確認なしで着手してPR作成まで進めてよい**（2026-09-06合意）: typo修正、文言1箇所の変更（規約・法務・事業判断に影響しないもの限定。ユーザーが個別に「様子見」等と保留判断した内容は対象外）、Linearチケット起票、ブログ記事の軽微な修正（誤字・数値更新・リンク修正等、記事の主張や構成を変えないもの）。この定義は「実装完了後の自動レビュー」のマージ確認バッチ化でも共通して使う。**note/X投稿の下書き生成はsns-hubパイプラインへの移行に伴い廃止済み（フローC-0参照）のため、このカテゴリには含めない**
...
- 1タスクが独立して大きい・複数ファイルにまたがり後のタスクと文脈を共有しない場合は、そのタスク単位で`spawn_task`または別セッションに切り出す（「セッション内のスコープ管理」の判断基準と同じ）。1つのセッションに全タスクを詰め込んで長時間化させない
