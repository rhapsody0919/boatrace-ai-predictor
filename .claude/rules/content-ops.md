...
### フローB-1: 新しいネタ種別は、チャネル別の生成物をユーザーが確認してから実運用に入る（2026-09-25〜）

新しいネタ種別（`sns_topic_categories` の行）を追加してパイプラインに載せただけでは**実運用開始としない**。**チャネルごとに実データで生成した投稿内容をユーザーに提示し、明示的なOKを得てから**日常運用に組み入れる。

きっかけ: 「本日のデータ一覧」型（`morning-digest`、BOA-402）を4チャネルに載せた際、ユーザーが実際の投稿イメージを見て初めて分かった問題が複数あった。**仕様書・プロンプトのレビューでは出てこなかった**類のもの。

- 文字数がプラットフォームの上限を超えていた（X無料枠は280 weight。日本語1文字=2、URLは23。X Premiumは見送り中で長文は投稿できない）
- 掲載順が画面と食い違っており、SNSに出るレースと画面に出るレースが別物になっていた
- 「同じ70%でも会場が違えば意味が違う。」のような、語呂合わせで作った不自然な日本語が混ざっていた
- 母数が小さい行（`is_small_sample`）を母数の併記なしで載せていた

**手順**:
1. その種別の実データで、**有効な全チャネル分**の投稿内容を生成して提示する（Routineを待たず、対話セッションで書いてよい）
2. プラットフォームの制約を**実測**する（文字数はコードで数える。「だいたい収まるはず」で出さない）
3. ユーザーの指摘を各チャネルのプロンプト（`docs/operation/sns-pipeline-*.md`）に反映する
4. **ユーザーのOKが出るまでは実運用としない**。その間も下書きの生成自体は続けてよい（提示する材料になるため）が、投稿はしない
5. 重すぎる・不要と判断されたチャネルは、コードを触らず sns-hub 管理画面「ネタ型設定」の `enabled` を落とす
