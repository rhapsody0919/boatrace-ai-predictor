...
## 原則
- **ヘルパー・ユーティリティを書く前に [`docs/reference/shared-logic-index.md`](../../docs/reference/shared-logic-index.md) を引く**（`src/utils`・`src/services`・`scripts/lib` の索引。ソースから機械生成し、CIで最新性を担保している）。実測で「既存実装を知らず再実装した」fixが26件あり、うち何件かは**同一ファイル内の別の関数**に既にあった
