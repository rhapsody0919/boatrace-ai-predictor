 ...
 *
 * 2026-08-23: 「答え合わせ型」（MascotCM.jsx）とは別の型をキャラAで試すため新設。
 * 実データ: /winning-technique の「本日の好調・不調選手ランキング」機能
 * （急上昇選手TOP10、本日出走の全選手を対象に現在の全国勝率と約90日前を比較）。
 * Playwrightで実画面をスクショ取得済み（tool-hot-cold-card.png）。
 */
...
// tool-hot-cold-card.png 実測値
const CARD_NATIVE_WIDTH = 372;
const CARD_NATIVE_HEIGHT = 658;
const CARD_DISPLAY_HEIGHT = 1360;
...
// 1位行(藤本紗弥香)のハイライト位置（実測: カード上部から見出し+ヘッダー分オフセット）
const HIGHLIGHT_REL = { x: 0, y: 108, width: 372, height: 64 };
const HIGHLIGHT_BOX = {
...
function SceneReveal({ mascotSrc }) {
  ...
  return (
    <AbsoluteFill style={{ background: NAVY, overflow: "hidden" }}>
      <PulseRings color={GREEN} size={900} top="92%" />
      <div
        style={{
          position: "absolute",
          top: 90,
      ...
      >
        <Img
          src={staticFile("tool-hot-cold-card.png")}
          style={{
            width: "100%",
            display: "block",
            borderRadius: 20,
