 ...
 *
 * 2026-08-25: マスコットテスト3日目（キャラC）の3本目。
 * 実データ: 芦屋8R（2026-08-25、締切11:53、結果未確定）。
 * AI逃げ確率33%・イン崩れ指数会場内100パーセンタイルという実際の予想数値。
 * Playwrightで実際の「イン崩れ注意度」カードをスクショ取得済み
 * （live-volatility-ashiya8r.png）。
 */
...
// live-volatility-ashiya8r.png 実測値（1116x294）
const CARD_NATIVE_WIDTH = 1116;
const CARD_NATIVE_HEIGHT = 294;
...
// 「100」の実測位置(x1044,y130,w36,h30) に余白を足したハイライト枠
const HIGHLIGHT_REL = { x: 1024, y: 108, width: 76, height: 70 };
const HIGHLIGHT_BOX = {
...
function SceneReveal({ mascotSrc }) {
  ...
  return (
    <AbsoluteFill style={{ background: NAVY, overflow: "hidden" }}>
      ...
      >
        <Img
          src={staticFile("live-volatility-ashiya8r.png")}
          style={{
            width: "100%",
            display: "block",
            borderRadius: 20,
...
export function LivePredictionCM_C() {
