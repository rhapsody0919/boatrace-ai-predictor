...
// 会場ガイドの観光情報が「質向上パターン」（2026-08-10確立、memory
// feedback_venue_guide_quality_upgrade 参照）に沿っているかを機械的に検証する。
// nearbyAttractionsを追加した会場（=質向上済み）のみが対象。
// 未着手の会場（フィールド自体が無い）はテスト対象外なので、横展開の進捗を妨げない。
test.describe("会場ガイドデータの品質チェック（zh-TW）", () => {
  for (const venue of VENUE_GUIDES_ZH_TW) {
    ...
    test(`${venue.slug}: 会場座標(lat/lng)が日本国内の範囲内`, () => {
      expect(venue.lat, `${venue.slug}.lat`).not.toBeNull();
      expect(venue.lng, `${venue.slug}.lng`).not.toBeNull();
      expect(
    ...
    test(`${venue.slug}: cashless情報が必須フィールドを満たす`, () => {
      expect(venue.cashless, `${venue.slug}にcashlessが無い`).toBeTruthy();
    ...
    test(`${venue.slug}: 会場座標がある場合、少なくとも半数以上の観光地にも座標がある`, () => {
